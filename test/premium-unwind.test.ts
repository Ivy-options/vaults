...
const load = fixture(connection, () => deployIvy(connection, { transfersEnabled: true }));
...
describe("activation premium and unanimous unwinds", () => {
  let c: IvyContext;
  beforeEach(async () => {
    c = await load();
  });

  it("allows unanimous cash unwind after the last publisher leaves without requiring a price report", async () => {
    const v = await goLive(
      c,
      { terms: { allowedSettlement: SettlementType.Cash, maxSettlementPriceAge: 3600 } },
    ...
    await expect(c.hub.connect(c.marketMaker).claimPayout(v.vaultId)).changeTokenBalance(
      c.ethers,
      c.usdc,
      c.marketMaker,
      100n * U,
  ...
  it("moves unclaimed income with shares and preserves it after burning", async () => {
  ...
  it("checkpoints duplicate-id batch transfers once and keeps floor dust reserved", async () => {
    ...
    await c.shares
      .connect(c.bob)
      .safeBatchTransferFrom(c.bob.address, c.carol.address, [v.vaultId, v.vaultId], [1n, 1n], "0x");
    ...
    await at(c, d.bid.expiry + 3600n);
    await c.hub.expire(d.vaultId);
    await c.hub.connect(c.alice).claim(d.vaultId, 1n);
    await c.hub.connect(c.bob).claim(d.vaultId, 2n);
    expect(await d.vault.reserved(c.usdcAddress)).eq(1n);
    expect(await c.usdc.balanceOf(d.vaultAddress)).eq(1n);
  });
  it("rejects unapproved, stale and unfunded unwind; preserves premium and funds a pull refund", async () => {
    const v = await goLive(c, {
      isCall: false,
      deposit: 18_000n * U,
      extraDeposits: [{ signer: c.bob, amount: 12_000n * U }],
    });
    await c.hub.connect(c.alice).claimPremium(v.vaultId);
  ...
  it("invalidates sender and receiver approvals on real transfers, but not zero or self transfers", async () => {
    const v = await goLive(c, { deposit: 6n * W, extraDeposits: [{ signer: c.bob, amount: 4n * W }] });
    ...
    await c.shares
      .connect(c.alice)
      .safeBatchTransferFrom(c.alice.address, c.bob.address, [v.vaultId, v.vaultId], [W, W], "0x");
    expect(await c.unwind.approvedShares(v.vaultId)).eq(0n);
    await expect(c.hub.executeUnwind(v.vaultId, a.nonce, signature)).revertedWithCustomError(
      c.unwind,
      "ConsentMissing",
    );
    for (const s of [c.alice, c.bob]) await c.hub.connect(s).approveUnwind(v.vaultId, a.nonce);
    ...
    expect(await c.unwind.approvedShares(v.vaultId)).eq(0n);
    await expect(c.hub.connect(c.alice).approveUnwind(v.vaultId, a.nonce)).revertedWithCustomError(
      c.unwind,
      "AgreementInvalid",
    );
  });
  it("partial exercise invalidates the signed snapshot; replacement preserves executed assets", async () => {
    const v = await goLive(c);
    ...
    await expect(c.hub.executeUnwind(v.vaultId, old.agreement.nonce, old.signature)).revertedWithCustomError(
      c.unwind,
      "AgreementInvalid",
    );
  ...
  it("only the owner or buyer proposes; bad signatures and expired agreements cannot unwind", async () => {
    const v = await goLive(c);
    await expect(c.hub.connect(c.bob).proposeUnwind(v.vaultId, v.bid.expiry, 0, "0x")).revertedWithCustomError(
      c.hub,
      "NotVaultOwner",
    );
    const a = await propose(c, v.vaultId, 0n);
    await c.hub.connect(c.alice).approveUnwind(v.vaultId, a.agreement.nonce);
    await expect(c.hub.executeUnwind(v.vaultId, a.agreement.nonce, "0x")).revertedWithCustomError(
      c.unwind,
      "BadSignature",
    );
    await networkHelpers.time.increaseTo(a.agreement.deadline + 1n);
    await expect(c.hub.executeUnwind(v.vaultId, a.agreement.nonce, a.signature)).revertedWithCustomError(
      c.unwind,
      "AgreementInvalid",
    );
  });
...
describe("admission and delegated execution", () => {
  let c: IvyContext;
  beforeEach(async () => {
    c = await load();
  });

  it("paused auctions cancel immediately and live options and premium claims remain usable", async () => {
    const v = await goLive(c);
    const o = await openVault(c);
    await expect(c.hub.connect(c.bob).setAdmissionPause(0, true)).revertedWithCustomError(
      c.hub,
  ...
  it("delegates fund exercise, cannot redirect, and lose access after revocation", async () => {
    const v = await goLive(c, {}, { executor: c.bob.address, recipient: c.carol.address });
    await expect(c.hub.connect(c.bob).exercise(v.vaultId, W)).revertedWithCustomError(
      c.usdc,
    ...
    await expect(c.hub.connect(c.bob).setExecution(v.vaultId, c.bob.address, c.bob.address)).revertedWithCustomError(
      c.hub,
      "NotMarketMaker",
    );
  ...
  it("binds the collateral, pair and auction identity, even after unchanged reopening", async () => {
    const o = await openVault(c);
    const b = await makeBid(c, o.vaultId);
    await fund(c, c.usdc, c.marketMaker, o.vaultAddress, 10000n * U);
    for (const bad of [
      { ...b, collateralAmount: b.collateralAmount + 1n },
      { ...b, termsHash: "0x" + "00".repeat(32) },
    ]) {
      await expect(
        c.hub.connect(c.bidMaster).activate(o.vaultId, bad, await signBid(c.marketMaker, c.hubAddress, bad)),
      ).revertedWithCustomError(c.hub, "CommitmentMismatch");
    }
    await c.hub.connect(c.bidMaster).cancelAuction(o.vaultId);
    await c.hub.connect(c.alice).openAuction(o.vaultId);
    await expect(
      c.hub.connect(c.bidMaster).activate(o.vaultId, b, await signBid(c.marketMaker, c.hubAddress, b)),
    ).revertedWithCustomError(c.hub, "CommitmentMismatch");
  });
  it("expired auctions cancel immediately but cannot reopen", async () => {
  ...
  it("premium plus buyer payout reserves coexist in put collateral", async () => {
    const v = await goLive(
      c,
