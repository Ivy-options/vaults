...
contract IvyUnwind is EIP712 {
    ...
    bytes32 public constant TYPEHASH = keccak256(
        "UnwindAgreement(uint256 vaultId,uint256 nonce,uint64 deadline,uint256 exercisedNotional,uint256 supply,uint256 refund)"
    );
    ...
    function revoke(uint256 id, address holder) external onlyHub {
        _invalidate(id, holder);
    }
    ...
    function fund(uint256 id, uint256 nonce, address holder, uint256 amount, uint256 exercised, uint256 revision)
        external
        onlyHub
    {
        UnwindAgreement storage a = agreements[id];
        if (
            nonce == 0 || nonce != a.nonce || block.timestamp > a.deadline || a.exercisedNotional != exercised
                || revisions[id][holder] != revision || amount == 0
        ) {
            revert AgreementInvalid();
        }
    ...
    function withdraw(uint256 id, uint256 nonce, address holder) external onlyHub returns (uint256 amount) {
        ...
        if (completion.executed) {
            ...
            uint256 surplus =
                weight == 0 ? 0 : Math.mulDiv(completion.remainingSurplus, weight, completion.remainingWeight);
