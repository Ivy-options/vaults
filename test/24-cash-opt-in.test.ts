...
describe("Cash settlement opt-in", function () {
  it("deploys with cash disabled and no settlement publishers", async function () {
    ...
    expect(await c.hub.settlementPublisherCount()).equal(0);
    ...
    await expect(c.hub.publishExercisePrice(c.wethAddress, c.usdcAddress, 4000n * U, now, now + 100n))
  ...
  it("counts actual publisher membership through duplicates, revocation and renunciation without counting other roles", async function () {
    const c = await defaultDeployment();
    const role = await c.hub.SETTLEMENT_PRICE_PUBLISHER_ROLE();
    const otherRole = await c.hub.BID_MASTER_ROLE();
    ...
    await c.hub.grantRole(otherRole, ZeroAddress);
    await c.hub.grantRole(otherRole, c.bob.address);
    await c.hub.revokeRole(otherRole, c.bob.address);
    expect(await c.hub.settlementPublisherCount()).equal(0);
    await c.hub.grantRole(role, c.bob.address);
    await c.hub.grantRole(role, c.bob.address);
    expect(await c.hub.settlementPublisherCount()).equal(1);
    expect(await c.hub.cashSettlementEnabled()).equal(true);
    await c.hub.grantRole(role, c.carol.address);
    expect(await c.hub.settlementPublisherCount()).equal(2);
    await c.hub.revokeRole(role, c.bob.address);
    await c.hub.revokeRole(role, c.bob.address);
    expect(await c.hub.settlementPublisherCount()).equal(1);
    expect(await c.hub.cashSettlementEnabled()).equal(true);
    await expect(c.hub.connect(c.bob).renounceRole(role, c.carol.address)).revertedWithCustomError(c.hub, "AccessControlBadConfirmation");
    expect(await c.hub.settlementPublisherCount()).equal(1);
    await c.hub.connect(c.carol).renounceRole(role, c.carol.address);
    await c.hub.connect(c.carol).renounceRole(role, c.carol.address);
    await c.hub.revokeRole(role, ZeroAddress);
    expect(await c.hub.settlementPublisherCount()).equal(0);
    expect(await c.hub.cashSettlementEnabled()).equal(false);
    expect(await c.hub.hasRole(role, c.carol.address)).equal(false);
  });

  it("enables cash through an EOA or authenticated helper role grant and preserves physical-only vault terms", async function () {
    const c = await defaultDeployment();
    const role = await c.hub.SETTLEMENT_PRICE_PUBLISHER_ROLE();
    ...
    expect(await c.hub.cashSettlementEnabled()).equal(true);
    const now = BigInt(await c.networkHelpers.time.latest());
    await expect(helper.connect(c.carol).publishExercisePrice(c.wethAddress, c.usdcAddress, 4000n * U, now, now + 100n))
      .revertedWithCustomError(helper, "OwnableUnauthorizedAccount");
    await helper.connect(c.bob).publishExercisePrice(c.wethAddress, c.usdcAddress, 4000n * U, now, now + 100n);
    await c.hub.createVault(callTerms(c, { allowedSettlement: SettlementPolicy.Cash, maxSettlementPriceAge: 3600 }), callPairs(c));
  ...
  it("rejects pending cash bids after the last publisher leaves without consuming the bid, and accepts them after regrant", async function () {
  ...
  it("still activates physical bids in existing Either auctions after the last publisher leaves", async function () {
    const c = await deployIvy(await network.create());
  ...
  it("preserves live cash exercise, finalization and claims after the last publisher is revoked", async function () {
    const c = await deployIvy(await network.create());
    const role = await c.hub.SETTLEMENT_PRICE_PUBLISHER_ROLE();
    ...
    await setExercisePrice(c, 4000n * U);
    await c.hub.revokeRole(role, c.admin.address);
    expect(await c.hub.cashSettlementEnabled()).equal(false);
    ...
    await publishExpiryPrice(c, v.bid.expiry, 6000n * U);
    await c.hub.revokeRole(role, c.admin.address);
  ...
  it("keeps missing cash reports locked while disabled and recovers through regrant and historical publication", async function () {
    ...
    await c.hub.grantRole(role, c.bob.address);
    const now = BigInt(await c.networkHelpers.time.latest());
    await c.hub.connect(c.bob).publishExpiry(c.wethAddress, c.usdcAddress, v.bid.expiry, 2700n * U, now + 100n);
