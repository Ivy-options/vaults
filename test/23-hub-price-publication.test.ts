...
describe("Hub-owned settlement prices", function () {
  ...
  it("accepts an authorized helper contract, protects its upstream caller, and revokes either publisher kind", async function () {
    ...
    await expect(helper.connect(c.carol).publishExercisePrice(v.vaultId, 4000, now, now + 100n))
      .revertedWithCustomError(helper, "OwnableUnauthorizedAccount");
    await expect(helper.connect(c.carol).publishExpiry(v.vaultId, 4000, now + 100n))
      .revertedWithCustomError(helper, "OwnableUnauthorizedAccount");
  ...
  it("grants publisher permissions separately from administration and isolates the same vault id between Hubs", async function () {
    ...
    await expect(other.hub.connect(other.bob).grantRole(role, other.carol.address)).revertedWithCustomError(other.hub, "AccessControlUnauthorizedAccount");
