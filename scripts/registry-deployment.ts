...
export interface RegistryDeploymentInput {
  artifact: Artifact;
...
export interface RegistryPlan extends PlanIdentity {
  version: number;
  kind: string;
  startNonce: number;
  admin: string;
...
export async function buildRegistryDeploymentPlan({
  artifact,
  chainId,
  genesisHash,
  deployer,
  startNonce,
  admin,
}: RegistryDeploymentInput): Promise<RegistryPlan> {
  ...
  return {
    ...
    steps: [
      {
        name: "IvyVaultsRegistry",
        address,
        nonce: startNonce,
        data: tx.data,
        deployedSize: (artifact.deployedBytecode.length - 2) / 2,
      },
    ],
  };
}
...
export async function resumeRegistryDeployment(
  signer: Signer,
  plan: RegistryPlan,
  artifact: Artifact,
  journal: Journal = {},
  ...
  if (json(await buildRegistryDeploymentPlan({ ...plan, artifact })) !== json(plan))
    throw new Error("Registry plan does not match this build");
  const provider = rpc(signer.provider);
  const { startBlock, steps } = await openJournal(signer, plan, journal);
  const step = plan.steps[0],
    entry = (steps.IvyVaultsRegistry ??= {});
  await persist(journal);
  if ((await currentCode(provider, plan.address)) !== "0x")
  ...
  if (!entry.hash) {
    ...
    entry.intent = { nonce, address: plan.address };
    await persist(journal);
