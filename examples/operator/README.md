...
- `deployment.json`: read-only `prepare-deployment` input. Save its output as deployment-plan.json. It predicts eight deployments, including the two linked libraries before the hub and the indicative feed. Deployment starts physical-only with zero settlement publishers. No publisher or settlement methodology is required to deploy; an optional settlementMethodology metadata reference may be saved in the plan.
...
- `cash-vault.json`: optional cash covered-call preparation after explicit publisher activation; includes the required methodology reference and positive observation age.
...
- `settlement-role.json`: `grant-settlement-publisher` or `revoke-settlement-publisher` input targeting the Hub, sent by its admin. Granting the first publisher enables new cash business; revoking the last disables new cash creation/activation.
...
To opt into cash, first have the Hub admin prepare and submit `grant-settlement-publisher` with `settlement-role.json`, naming an EOA or an existing authenticated helper. Verify `cashSettlementEnabled()` is true and `settlementPublisherCount()` is positive. Then use `cash-vault.json`. No deployment step grants the role automatically.
...
Deployment manifests use version 5 and eight creation transactions. Generate a new plan for this build; do not reuse the earlier standalone-feed plan. There is no default publisher and no settlement helper is deployed. The default `vault.json` is physical-only and requires no settlement methodology.

Revoking or renouncing the last publisher stops new cash-capable vault creation and cash activation. Existing cash options retain exercise, expiration and claim rights using already-stored prices; missing reports still require publisher recovery or the existing unanimous unwind. Physical vaults remain available.
