...
Before granting the first production publisher, approve a methodology artifact for each pair naming the responsible operator, source venues, sampling frequency, weighting, rounding, missing-data/outage rules, evidence retention and freshness configuration. The expiry methodology uses a 30-minute average ending at exact expiry; exercise observations need an approved current-price calculation and validity policy. Reference the artifact using `settlementMethodology` in cash-vault preparation and report-publication requests. Physical-only deployment requires no methodology. This records operational context; the contracts and CLI do not verify source accuracy or calculate that average. Synthetic examples are for local rehearsal only.

### Enable cash after physical launch

Cash support starts disabled because the Hub has no publisher members. After preparing the cash methodology and operator, the Hub admin uses `grant-settlement-publisher` with the Hub address and the nonzero publisher account. Verify that `settlementPublisherCount()` is positive and `cashSettlementEnabled()` is true before creating Cash or Either vaults. Granting the first member enables these admissions immediately; there is no separate switch. The count proves authorization, not that the publisher is online.

At zero members, new Cash/Either vault creation and every cash bid activation revert. Activation rechecks availability even for an older auction. Physical bids still work, including in an existing Either vault. Existing physical-only terms cannot be loosened to Cash/Either after enablement. Removing the last member also leaves deposits, tightening and auction cancellation under their existing rules.

Revoke a publisher through `revoke-settlement-publisher`; holders may also use AccessControl `renounceRole`. Both update the member count. Duplicate grants/revocations and unrelated roles do not change it. Removing the last publisher disables new cash admissions but preserves exercise, expiration, claims and agreed unwind for existing positions. Stored reports retain their validity/finality rules; missing reports require a replacement publisher. No balances are released or obligations erased by disabling cash.

### EOA and optional contract publishers

For the direct EOA path, grant the EOA the Hub’s publisher role after deployment. That EOA submits `publish-settlement-exercise` and `publish-settlement-expiry` to the Hub. No settlement feed address, interface implementation or helper deployment is needed. Matching vaults on the Hub read the same report keyed by ordered pair and expiry.
