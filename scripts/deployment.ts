...
export interface DeploymentStep extends CreationStep {
  abi: InterfaceAbi;
...
export interface JournalEntry {
  hash?: string;
  runtimeHash?: string;
  intent?: { nonce: number; address: string };
}
...
export interface DeploymentInput {
  ...
  reportSigner: string;
  settlementMethodology?: string;
  exerciseWindow?: BigNumberish;
  expiryPricePublicationWindow?: BigNumberish;
  auctionTimeout?: BigNumberish;
  uri?: string;
}
...
export const artifactPath = (name: string) =>
  `../artifacts/contracts/${LIBRARIES.includes(name) ? "libraries/" : ""}${name}.sol/${name}.json`;
export async function loadArtifacts(): Promise<Artifacts> {
  return Object.fromEntries(
    await Promise.all(
      CONTRACTS.map(async (name) => [
        name,
        JSON.parse(await readFile(new URL(artifactPath(name), import.meta.url), "utf8")),
      ]),
    ),
  );
}
...
export function linkBytecode(
  artifact: Pick<Artifact, "bytecode" | "linkReferences">,
  addresses: Record<string, string>,
) {
...
function runtimeLinks(artifact: Artifact, addresses: Record<string, string>): LibraryLink[] {
  return Object.values(artifact.deployedLinkReferences ?? {}).flatMap((names) =>
    Object.entries(names).flatMap(([name, refs]) =>
      refs.map(({ start, length }) => {
...
export async function buildDeploymentPlan({
  ...
  exerciseWindow,
  expiryPricePublicationWindow,
  auctionTimeout = 259200,
  uri = "",
}: DeploymentInput): Promise<DeploymentPlan> {
  if (expiryPricePublicationWindow === undefined || BigInt(expiryPricePublicationWindow) <= 0n)
    throw new Error("Explicit positive expiryPricePublicationWindow required");
  if (exerciseWindow === undefined || BigInt(exerciseWindow) < 0n)
    throw new Error("Explicit nonnegative exerciseWindow required; cash-capable vaults require a positive value");
  const addresses = Object.fromEntries(
    CONTRACTS.map((name, i) => [name, getCreateAddress({ from: deployer, nonce: startNonce + i })]),
  );
  const a = addresses;
  const args = [
    [],
  ...
  for (let i = 0; i < CONTRACTS.length; ++i) {
    ...
    steps.push({
      ...
      abi,
      deployedSize: size,
      libraryLinks: runtimeLinks(artifact, a),
    });
  }
  return {
    version: 7,
    ...
    reportSigner,
    settlementMethodology,
    exerciseWindow: String(exerciseWindow),
    expiryPricePublicationWindow: String(expiryPricePublicationWindow),
    auctionTimeout: String(auctionTimeout),
    uri,
    addresses,
    steps,
  };
...
export async function findCreation(
  ...
  if (startBlock === undefined || !Number.isSafeInteger(startBlock) || startBlock < 0 || startBlock > latest)
    throw new Error("Invalid creation scan start block");
...
export const currentCode = (provider: RpcProvider, address: string): Promise<string> =>
...
export async function verifyCreation(provider: RpcProvider, plan: PlanIdentity, step: CreationStep, hash: string) {
  ...
  if (
    ...
    receipt.status !== 1 ||
    tx.to !== null ||
    tx.from.toLowerCase() !== plan.deployer.toLowerCase() ||
    tx.nonce !== step.nonce ||
    tx.data !== step.data ||
    tx.value !== 0n ||
    tx.chainId.toString() !== plan.chainId ||
    receipt.contractAddress?.toLowerCase() !== step.address.toLowerCase()
  )
    throw new Error(`Creation evidence mismatch: ${step.name}`);
...
export async function verifyBindings(
  ...
  for (const [name, field, value] of checks)
    if ((await byName[name].getFunction(field)()).toLowerCase() !== value.toLowerCase())
      throw new Error(`Binding mismatch: ${name}.${field}`);
  ...
  for (const step of plan.steps) {
    ...
    for (const link of step.libraryLinks) {
      ...
      if (
        embedded.toLowerCase() !== a[link.name].toLowerCase() ||
        link.address.toLowerCase() !== a[link.name].toLowerCase()
      ) {
  ...
  if (requireInitialAdmin && !(await hub.hasRole(await hub.DEFAULT_ADMIN_ROLE(), plan.admin)))
...
export async function openJournal(
  ...
  plan: PlanIdentity,
  journal: Journal,
): Promise<Required<Pick<Journal, "startBlock" | "steps">>> {
  const provider = rpc(signer.provider);
  if (
    String((await provider.getNetwork()).chainId) !== plan.chainId ||
    (await provider.getBlock(0))?.hash !== plan.genesisHash
  )
  ...
  for (const step of plan.steps) {
    ...
    if (existing !== "0x") {
      ...
      if (!entry.hash) {
        ...
        entry.intent = { nonce: step.nonce, address: step.address };
        await persist(journal);
