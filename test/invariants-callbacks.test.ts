...
describe("cross-module callbacks and reserve invariants", () => {
  let c: IvyContext;
  beforeEach(async () => {
    c = await load();
  });
  it("checks holder revision after contribution-token callbacks and rolls back funding and share movement", async () => {
    ...
    await token.mint(c.marketMaker.address, 1000n * U);
    await token.connect(c.marketMaker).approve(v.vaultAddress, 1000n * U);
    await activate(c, v.vaultId, v.vaultAddress);
    const { agreement: a, signature: sig } = await proposeUnwind(
      c,
      v.vaultId,
    ...
    await token.mint(c.alice.address, a.refund);
    await token.connect(c.alice).approve(v.vaultAddress, a.refund);
    await c.shares.connect(c.alice).setApprovalForAll(await token.getAddress(), true);
    await token.arm(
      c.sharesAddress,
      c.shares.interface.encodeFunctionData("safeTransferFrom", [c.alice.address, c.carol.address, v.vaultId, W, "0x"]),
    );
    await expect(c.hub.connect(c.alice).fundUnwind(v.vaultId, a.nonce, a.refund)).revertedWithCustomError(
      c.unwind,
      "AgreementInvalid",
    );
    ...
    await token.arm(
      ...
      token.interface.encodeFunctionData("roundTrip", [c.sharesAddress, c.alice.address, v.vaultId, W]),
    );
    await expect(c.hub.connect(c.alice).fundUnwind(v.vaultId, a.nonce, a.refund)).revertedWithCustomError(
      c.unwind,
      "AgreementInvalid",
    ...
    await token.disarm();
    await c.shares.connect(c.alice).safeTransferFrom(c.alice.address, c.carol.address, v.vaultId, W, "0x");
    ...
    await token.mint(c.carol.address, 10n * U);
    await token.connect(c.carol).approve(v.vaultAddress, 10n * U);
  ...
  it("snapshots premium before collection callbacks can transfer shares", async () => {
    ...
    await token.arm(
      c.sharesAddress,
      c.shares.interface.encodeFunctionData("safeTransferFrom", [
        c.alice.address,
        c.carol.address,
        v.vaultId,
        10n * W,
  ...
  it("guards direct vault deposits against nested transfers that would inflate balance-delta credit", async () => {
    ...
    await token.mint(c.alice.address, 100);
    await token.mint(address, 10);
    await token.connect(c.alice).approve(v.vaultAddress, 100);
    await token.approveSelf(v.vaultAddress, 10);
  ...
  it("enforces independent premium caps even when premium and collateral are the same token", async () => {
    const mock = await ethers.deployContract("MockHub");
    await mock.createClone(c.vaultImplAddress, 1, c.usdcAddress);
    const address = await mock.lastClone(),
      v = await ethers.getContractAt("IvyVault", address);
    await c.usdc.mint(address, 1000);
    await c.usdc.mint(c.alice.address, 100);
    await c.usdc.connect(c.alice).approve(address, 100);
    await mock.collectPremium(address, c.usdcAddress, c.alice.address, 100);
    await mock.reserveBuyer(address, c.usdcAddress, 200);
    ...
    await expect(mock.push(address, c.usdcAddress, c.alice.address, 801)).revertedWithCustomError(
      v,
      "InsufficientAvailable",
    );
    await expect(mock.collectPremium(address, c.usdcAddress, c.alice.address, 0)).revertedWithCustomError(
      v,
      "AlreadyInitialized",
    );
    await mock.payBuyer(address, c.usdcAddress, c.bob.address);
    expect(await v.reserved(c.usdcAddress)).eq(100n);
    await mock.payPremium(address, c.alice.address, 100);
    await mock.push(address, c.usdcAddress, c.alice.address, 800);
    expect(await c.usdc.balanceOf(address)).eq(0n);
  });
  for (const isCall of [true, false])
    for (const cash of [true, false])
      it(`stateful conservation: ${isCall ? "call" : "put"}, ${cash ? "cash" : "physical"}`, async () => {
        const unit = isCall ? W : 3000n * U;
        const v = await goLive(
          c,
          { isCall, withFeed: cash, deposit: 6n * unit, extraDeposits: [{ signer: c.bob, amount: 4n * unit }] },
          { settlement: cash ? SettlementType.Cash : SettlementType.Physical, style: ExerciseStyle.American },
        );
        const holders = [c.alice, c.bob, c.carol],
          balances = [6n * unit, 4n * unit, 0n];
        let paid = 0n,
          seed = 19;
        for (let i = 0; i < 30; i++) {
          ...
          await c.shares
            .connect(holders[from])
            .safeTransferFrom(holders[from].address, holders[to].address, v.vaultId, amount, "0x");
          balances[from] -= amount;
          balances[to] += amount;
          if (i === 8 || i === 17) {
            const h = i === 8 ? c.alice : c.bob;
            const due = await c.premiums.claimable(v.vaultId, h.address);
            await c.hub.connect(h).claimPremium(v.vaultId);
            paid += due;
          }
        ...
        if (cash) {
          await publishExpiryPrice(c, v.vaultId, isCall ? 3300n * U : 2700n * U);
        ...
        for (let i = 0; i < 3; i++) {
          if (balances[i] > 0n) await c.hub.connect(holders[i]).claim(v.vaultId, balances[i]);
          if ((await c.premiums.claimable(v.vaultId, holders[i].address)) > 0n)
            await c.hub.connect(holders[i]).claimPremium(v.vaultId);
        }
        ...
        expect(
          (await c.usdc.balanceOf(c.alice.address)) +
            (await c.usdc.balanceOf(c.bob.address)) +
            (await c.usdc.balanceOf(c.carol.address)) +
            (cash && !isCall ? 3000n * U : 0n),
        ).eq(isCall ? (cash ? 1000n * U : 13000n * U) : cash ? 31000n * U : 19000n * U);
      });
});
