...
import { deployIvy, SettlementType, WETH_UNIT as W, USDC_UNIT as U, fixture } from "./helpers/setup.js";
import type { IvyContext } from "./helpers/setup.js";
import { goLive, at } from "./helpers/scenarios.js";

const connection = await network.create();
const load = fixture(connection, () => deployIvy(connection, { enableCashSettlement: false }));

describe("Explicit cash feature flag", () => {
  let c: IvyContext;
  beforeEach(async () => {
    c = await load();
  });

  it("requires an explicit administrator toggle independently of publisher membership", async () => {
    ...
    await expect(c.hub.connect(c.bob).setCashSettlementEnabled(true)).revertedWithCustomError(
      c.hub,
...
describe("Per-vault settlement prices", () => {
  ...
  it("isolates exercise and final prices for matching vaults with separate publication transactions", async () => {
    ...
    await expect(c.hub.connect(c.marketMaker).exercise(second.vaultId, W)).revertedWithCustomError(
      c.hub,
      "InvalidPrice",
    );
    await c.hub.publishExercisePrice(second.vaultId, 6000n * U, now, now + 100n);
    await expect(c.hub.connect(c.marketMaker).exercise(first.vaultId, W)).changeTokenBalance(
      c.ethers,
      c.weth,
      c.marketMaker,
      W / 4n,
    );
    await expect(c.hub.connect(c.marketMaker).exercise(second.vaultId, W)).changeTokenBalance(
      c.ethers,
      c.weth,
      c.marketMaker,
      W / 2n,
    ...
    await expect(c.hub.publishExpiry(first.vaultId, 7000n * U, c.defaultExpiry + 100n)).revertedWithCustomError(
      c.hub,
      "ReportFinalized",
    );
