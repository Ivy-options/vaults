...
const premiumCollected = fixture(signerHub, async c => {
	...
	await (await c.vault.connect(c.hubSigner).collectPremium(c.tokenAddress, c.alice.address, weth(4), weth(1), c.treasury.address)).wait()
	return c
})
...
describe("IvyVault", () => {
	...
	describe("initialize", () => {
		context("once MockHub has cloned the vault", () => {
			...
			it("reverts on the implementation, which is locked at construction", async () => {
				await expect(c.impl.initialize(c.mockHubAddress, 1n, c.tokenAddress, c.mockHubAddress)).to.be.revertedWithCustomError(
					c.impl,
					"AlreadyInitialized",
				)
			})
			...
			it("reverts a second initialization of a clone", async () => {
				await expect(c.vault.initialize(c.mockHubAddress, 2n, c.tokenAddress, c.mockHubAddress)).to.be.revertedWithCustomError(
					c.vault,
					"AlreadyInitialized",
				)
			})
	...
	describe("pull", () => {
		...
		it("reverts for anyone but the hub", async () => {
			await expect(c.vault.connect(c.stranger).pull(c.tokenAddress, c.alice.address, 1n)).to.be.revertedWithCustomError(c.vault, "NotHub")
		})
	...
	describe("push", () => {
		context("after the hub pulls in 2 WETH", () => {
			...
			it("reverts for anyone but the hub", async () => {
				await expect(c.vault.connect(c.stranger).push(c.tokenAddress, c.stranger.address, 1n)).to.be.revertedWithCustomError(c.vault, "NotHub")
			})
	...
	describe("collectPremium", () => {
		context("before any premium is collected", () => {
			...
			it("rejects a fee one wei above the premium", async () => {
				await expect(
					c.vault.connect(c.hubSigner).collectPremium(c.tokenAddress, c.alice.address, weth(1), weth(1) + 1n, c.treasury.address),
				).to.be.revertedWithCustomError(c.vault, "InvalidPlatformFee")
			})

			it("accepts a fee equal to the premium and leaves the LPs nothing", async () => {
				await approveVault(weth(1))
				await c.vault.connect(c.hubSigner).collectPremium(c.tokenAddress, c.alice.address, weth(1), weth(1), c.treasury.address)
			...
			it("moves no tokens for a zero premium", async () => {
				await expect(c.vault.connect(c.hubSigner).collectPremium(c.tokenAddress, c.alice.address, 0n, 0n, c.treasury.address)).not.to.emit(
					c.token,
					"Transfer",
				)
			})
	...
	describe("claimPlatformFee", () => {
		context("after collecting a 4 WETH premium with a 1 WETH fee", () => {
			...
			it("emits PlatformFeeClaimed with the treasury and fee", async () => {
				await expect(c.vault.connect(c.stranger).claimPlatformFee()).to.emit(c.vault, "PlatformFeeClaimed").withArgs(c.treasury.address, weth(1))
			})
		})
	})
	...
	describe("payBuyer", () => {
		context("with nothing reserved for the buyer", () => {
			...
			it("moves no tokens", async () => {
				await expect(c.mockHub.payBuyer(c.vaultAddress, c.tokenAddress, c.alice.address)).not.to.emit(c.token, "Transfer")
			})
		})
	})
