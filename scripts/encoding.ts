...
const fields = (entries: [name: string, type: string][]): TypedDataField[] =>
...
export const encodeSpotBand = (priceFeed: AddressLike, maxPriceAge: BigNumberish, maxInTheMoneyBps: BigNumberish) =>
  coder.encode(
    ["tuple(address priceFeed,uint32 maxPriceAge,uint16 maxInTheMoneyBps)"],
    [[priceFeed, maxPriceAge, maxInTheMoneyBps]],
  );
export const encodePremiumFloor = (priceFeed: AddressLike, maxPriceAge: BigNumberish, minPremiumBps: BigNumberish) =>
  coder.encode(
    ["tuple(address priceFeed,uint32 maxPriceAge,uint16 minPremiumBps)"],
    [[priceFeed, maxPriceAge, minPremiumBps]],
  );
