...
- American cash early exercise uses a fresh, unexpired observation from the separate authoritative settlement feed before expiry, as specified in [settlement-pricing-spec.md](settlement-pricing-spec.md). At or after expiry, cash exercise uses the finalized expiry price, including for European cash options. Cash expiration is available at or after expiry. Finalized expiry reports remain required; current spot must not replace them.
