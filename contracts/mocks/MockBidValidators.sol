...
contract ContextAssertingValidator is IIvyBidValidator {
    ...
    function validateBid(bytes4, BidContext calldata context, Bid calldata bid, bytes calldata)
        external
        view
        returns (bytes4)
    {
        if (
            context.premiumToken != expectedPremiumToken || context.totalNotional != expectedTotalNotional
                || context.vaultId != bid.vaultId || context.collateralAmount != bid.collateralAmount
