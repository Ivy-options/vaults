	...
	describe("activate", () => {
		context("before a pool exists", () => {
			...
			it("rejects a zero supply", async () => {
				await expect(c.premiums.connect(c.hub).activate(VAULT_ID, c.vault.address, 10n, 0n)).to.be.revertedWithCustomError(c.premiums, "ZeroAmount")
			})
		})
		...
		context("once a pool is active", () => {
			...
			it("rejects a second activation", async () => {
				await expect(c.premiums.connect(c.hub).activate(VAULT_ID, c.vault.address, 10n, 3n)).to.be.revertedWithCustomError(
					c.premiums,
					"AlreadyInitialized",
				)
			})
