...
contract IvyVaultsRegistry is AccessControl {
    ...
    struct Release { address hub; bytes32 manifestHash; }
    ...
    function hubOf(uint256 releaseId) external view returns (address) { return _release(releaseId).hub; }
    function manifestHashOf(uint256 releaseId) external view returns (bytes32) { return _release(releaseId).manifestHash; }
