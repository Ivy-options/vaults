...
The filesystem-free `resolveRelease` export in `scripts/releases.mjs` accepts a provider, a request containing the parsed `releaseBundle`, the supported artifacts, and an optional `{ allowRecommended: true }` for new-position discovery. It returns the concrete Hub, chain, registry, release ID, bundle hash and saved artifacts. Browser integrations load the JSON bundle themselves; the CLI additionally supports `releaseBundleFile`. Use the returned saved interfaces for contract calls and preserve the Hub address when indexing the creation receipt.
