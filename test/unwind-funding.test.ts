...
import { deployIvy, fixture, fund, WETH_UNIT as W, USDC_UNIT as U } from "./helpers/setup.js";
...
async function proposal(c: IvyContext, id: bigint, refund: bigint) {
...
describe("LP-funded unwind", () => {
  let c: IvyContext;
  beforeEach(async () => {
    c = await load();
  });

  it("requires each current LP to fund their share even after claiming premium", async () => {
    ...
    await expect(c.hub.executeUnwind(v.vaultId, a.nonce, a.signature)).revertedWithCustomError(
      c.unwind,
      "FundingMissing",
    );
  ...
  for (const invalidate of ["revoke", "replace", "deadline", "exercise", "settle"] as const) {
    it(`returns individually reserved funds after ${invalidate}, even after burning all shares`, async () => {
      ...
      if (invalidate === "exercise") {
        await fund(c, c.weth, c.marketMaker, v.vaultAddress, W);
      ...
      await expect(c.hub.connect(c.alice).withdrawUnwindContribution(v.vaultId, a.nonce)).revertedWithCustomError(
        c.unwind,
        "NothingToClaim",
      );
  ...
  it("requires the new owner to fund and preserves the former holder's entire contribution", async () => {
  ...
  it("recovers ceiling surplus without changing the signed refund, even after transfers and burns", async () => {
    ...
    await expect(c.hub.connect(c.carol).withdrawUnwindContribution(v.vaultId, a.nonce)).changeTokenBalance(
      c.ethers,
      c.usdc,
      c.carol,
      1n,
  ...
  it("recovers deposits for zero-refund agreements and allows withdrawing active funding to revoke consent", async () => {
    const v = await goLive(c);
  ...
  it("keeps replacement funding separate and recalculates eligibility after partial transfers and topups", async () => {
    const v = await goLive(c, { deposit: 6n * W, extraDeposits: [{ signer: c.bob, amount: 4n * W }] });
    ...
    await c.hub.connect(c.alice).fundUnwind(v.vaultId, a.nonce, 10n * U);
    await c.hub.executeUnwind(v.vaultId, a.nonce, a.signature);
    await expect(c.hub.connect(c.alice).withdrawUnwindContribution(v.vaultId, old.nonce)).changeTokenBalance(
      c.ethers,
      c.usdc,
      c.alice,
      60n * U,
    );
    await expect(c.hub.connect(c.bob).withdrawUnwindContribution(v.vaultId, a.nonce)).changeTokenBalance(
      c.ethers,
      c.usdc,
      c.bob,
      20n * U,
