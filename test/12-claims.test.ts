...
describe("claim", function () {
  ...
  it("expired physical call: LPs split collateral and premium pro-rata and the vault empties", async function () {
    ...
    await expect(txA).to.changeTokenBalances(ethers, usdc, [alice], [600n * USDC_UNIT]);

    const txB = hub.connect(bob).claim(vaultId, 4n * WETH_UNIT);
    await expect(txB).to.changeTokenBalances(ethers, weth, [bob], [4n * WETH_UNIT]);
    await expect(txB).to.changeTokenBalances(ethers, usdc, [bob], [400n * USDC_UNIT]);
  ...
  it("fully exercised physical call: LPs receive quote plus premium", async function () {
    ...
    const txA = hub.connect(alice).claim(vaultId, 6n * WETH_UNIT);
    await expect(txA).to.changeTokenBalances(ethers, usdc, [alice], [18_600n * USDC_UNIT]);
    await expect(txA).to.changeTokenBalances(ethers, weth, [alice], [0n]);
    await expect(hub.connect(bob).claim(vaultId, 4n * WETH_UNIT)).to.changeTokenBalances(ethers, usdc, [bob], [12_400n * USDC_UNIT]);
    expect(await usdc.balanceOf(vaultAddress)).to.equal(0n);
  });
  ...
  it("partially exercised put: LPs receive a mixed pot", async function () {
    ...
    await expect(txA).to.changeTokenBalances(ethers, usdc, [alice], [11_400n * USDC_UNIT]);
    ...
    await expect(txB).to.changeTokenBalances(ethers, usdc, [bob], [7_600n * USDC_UNIT]);
  ...
  it("cash settlement excludes the market maker's pending payout", async function () {
    ...
    await expect(tx).to.changeTokenBalances(ethers, usdc, [alice], [1000n * USDC_UNIT]);
  ...
  it("transferred shares can claim", async function () {
    ...
    await expect(tx).to.changeTokenBalances(ethers, usdc, [carol], [200n * USDC_UNIT]);
  });
  ...
  it("partial claims stay proportional", async function () {
    ...
    await expect(first).to.changeTokenBalances(ethers, usdc, [alice], [300n * USDC_UNIT]);
    ...
    await expect(second).to.changeTokenBalances(ethers, usdc, [alice], [300n * USDC_UNIT]);
