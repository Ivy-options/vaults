...
import { deployIvy, fixture, fund, WETH_UNIT as W, USDC_UNIT as U, Phase } from "./helpers/setup.js";
...
describe("buyer-authenticated unwind proposals", () => {
  let c: IvyContext;
  beforeEach(async () => {
    c = await load();
  });

  it("rejects an outsider with a fresh valid buyer signature without consuming its nonce", async () => {
    ...
    for (const caller of [c.alice, c.marketMaker]) {
      ...
      await expect(c.hub.connect(c.bob).proposeUnwind(v.vaultId, deadline, 0n, p.signature)).revertedWithCustomError(
        c.hub,
        "NotVaultOwner",
      );
  ...
  it("preserves an executable proposal when a non-shareholding owner attempts an unauthorized replacement", async () => {
    const v = await goLive(c);
    ...
    await expect(c.hub.connect(c.alice).proposeUnwind(v.vaultId, deadline, 0n, "0x")).revertedWithCustomError(
      c.unwind,
      "BadSignature",
    );
  ...
  it("binds every agreement field and EIP-712 domain before accepting either authorized caller", async () => {
    const v = await goLive(c);
    const deadline = BigInt(await networkHelpers.time.latest()) + 1000n;
    const p = await signUnwindProposal(c, v.vaultId, deadline, 0n);
    expect(p.digest).eq(TypedDataEncoder.hash(p.domain, UNWIND_TYPES, p.agreement));
    expect(p.agreement).deep.eq({
      vaultId: v.vaultId,
      nonce: 1n,
      deadline,
    ...
    for (const caller of [c.alice, c.marketMaker]) {
      await expect(c.hub.connect(caller).proposeUnwind(v.vaultId, deadline, 0n, "0x")).revertedWithCustomError(
        c.unwind,
        "BadSignature",
      );
      for (const [field, value] of Object.entries(p.agreement)) {
        const signature = await c.marketMaker.signTypedData(p.domain, UNWIND_TYPES, {
          ...p.agreement,
          [field]: value + 1n,
        });
        await expect(c.hub.connect(caller).proposeUnwind(v.vaultId, deadline, 0n, signature)).revertedWithCustomError(
          c.unwind,
          "BadSignature",
        );
      }
      for (const domain of [
        { ...p.domain, chainId: p.domain.chainId + 1n },
        { ...p.domain, verifyingContract: c.hubAddress },
      ]) {
    ...
    await expect(
      c.hub.connect(c.alice).proposeUnwind(v.vaultId, deadline, 0n, outsiderSignature),
    ...
    await expect(c.hub.connect(c.alice).proposeUnwind(v.vaultId, deadline, 0n, p.signature)).revertedWithCustomError(
      c.unwind,
      "BadSignature",
    );
  ...
  it("requires a new signature after an intervening partial exercise", async () => {
    const v = await goLive(c);
    const deadline = BigInt(await networkHelpers.time.latest()) + 1000n;
    const p = await signUnwindProposal(c, v.vaultId, deadline, 0n);
  ...
  it("rejects creation at its exact deadline but permits execution at the signed deadline", async () => {
    const v = await goLive(c);
    const deadline = BigInt(await networkHelpers.time.latest()) + 1000n;
    const p = await signUnwindProposal(c, v.vaultId, deadline, 0n);
  ...
  it("requires buyer authorization even when replacing an expired proposal", async () => {
    const v = await goLive(c);
    const deadline = BigInt(await networkHelpers.time.latest()) + 1000n;
    await proposeUnwind(c, v.vaultId, deadline, 0n);
    await networkHelpers.time.increaseTo(deadline + 1n);
    await expect(c.hub.connect(c.alice).proposeUnwind(v.vaultId, deadline + 1000n, 0n, "0x")).revertedWithCustomError(
      c.unwind,
      "BadSignature",
    );
    const fresh = await proposeUnwind(c, v.vaultId, deadline + 1000n, 0n);
