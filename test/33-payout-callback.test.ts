...
import { ExerciseStyle, Phase, SettlementType, USDC_UNIT as U, WETH_UNIT as W, deployIvy, fund } from "./helpers/setup.js";
...
describe("payout callback", function () {
  ...
  it("does not notify an externally owned recipient", async function () {
  ...
  it("notifies a contract recipient after the tokens arrive", async function () {
    const c = await networkHelpers.loadFixture(fixture);
    const { vaultId, vaultAddress } = await physicalCall(c);
    await fund(c, c.usdc, c.marketMaker, vaultAddress, 12_000n * U);
    const tx = c.hub.connect(c.marketMaker).exercise(vaultId, 4n * W);
    await expect(tx).to.emit(c.hub, "PayoutNotified").withArgs(vaultId, c.receiverAddress, c.wethAddress, 4n * W, true);
  ...
  it("runs after the vault is finalized on the last exercise", async function () {
    const c = await networkHelpers.loadFixture(fixture);
    const { vaultId, vaultAddress } = await physicalCall(c);
    ...
    await expect(tx).to.emit(c.hub, "PayoutNotified").withArgs(vaultId, c.receiverAddress, c.wethAddress, 10n * W, true);
    expect(await c.receiver.phaseSeen()).to.equal(Phase.Settled);
  });

  it("notifies the recipient chosen later through setExecution", async function () {
    const c = await networkHelpers.loadFixture(fixture);
    const { vaultId, vaultAddress } = await goLive(c);
    ...
    await expect(c.hub.connect(c.bob).exercise(vaultId, W))
      .to.emit(c.hub, "PayoutNotified").withArgs(vaultId, c.receiverAddress, c.wethAddress, W, true);
  });

  it("records an unacknowledged call when the recipient lacks the hook", async function () {
    const c = await networkHelpers.loadFixture(fixture);
  ...
  for (const [label, mode] of [["reverts", Mode.Revert], ["returns the wrong magic value", Mode.WrongMagic]] as const) {
    it(`still pays when the recipient ${label}`, async function () {
      const c = await networkHelpers.loadFixture(fixture);
      const { vaultId, vaultAddress } = await physicalCall(c);
  ...
  it("reverts when the hook runs out of gas so estimates stay honest", async function () {
    const c = await networkHelpers.loadFixture(fixture);
    const { vaultId, vaultAddress } = await physicalCall(c);
    await fund(c, c.usdc, c.marketMaker, vaultAddress, 3_000n * U);
    await c.receiver.setMode(Mode.BurnGas);
    await expect(c.hub.connect(c.marketMaker).exercise(vaultId, W, { gasLimit: 1_000_000 }))
  ...
  it("notifies a cash exercise payout in the collateral token", async function () {
    const c = await networkHelpers.loadFixture(fixture);
    const { vaultId } = await goLive(c, { withFeed: true }, { settlement: SettlementType.Cash, recipient: c.receiverAddress });
    ...
    await expect(c.hub.connect(c.marketMaker).exercise(vaultId, 10n * W))
      .to.emit(c.hub, "PayoutNotified").withArgs(vaultId, c.receiverAddress, c.wethAddress, payout, true);
    expect(await c.receiver.balanceSeen()).to.equal(payout);
    expect(await c.receiver.phaseSeen()).to.equal(Phase.Settled);
  });

  it("notifies a claimed cash payout", async function () {
    const c = await networkHelpers.loadFixture(fixture);
    const { vaultId, bid } = await goLive(c, { withFeed: true }, { settlement: SettlementType.Cash, style: ExerciseStyle.European, recipient: c.receiverAddress });
  ...
  it("notifies an unwind refund in the premium token", async function () {
    const c = await networkHelpers.loadFixture(fixture);
    await c.hub.setTransfersEnabled(true);
    const { vaultId, vaultAddress } = await physicalCall(c);
    const { agreement, signature } = await proposeUnwind(c, vaultId, BigInt(await networkHelpers.time.latest()) + 86400n, 100n * U);
    ...
    const tx = c.hub.connect(c.marketMaker).claimPayout(vaultId);
    await expect(tx).to.emit(c.hub, "PayoutNotified").withArgs(vaultId, c.receiverAddress, c.usdcAddress, 100n * U, true);
    await expect(tx).to.changeTokenBalances(ethers, c.usdc, [c.receiverAddress], [100n * U]);
    expect(await c.receiver.calls()).to.equal(1n);
  });

  it("blocks reentry from the hook and still completes the claim", async function () {
    const c = await networkHelpers.loadFixture(fixture);
    const { vaultId, bid } = await goLive(c, { withFeed: true }, { settlement: SettlementType.Cash, style: ExerciseStyle.European, recipient: c.receiverAddress });
    ...
    const payout = (10n * W * 300n) / 3300n;
    const tx = c.hub.connect(c.marketMaker).claimPayout(vaultId);
    await expect(tx).to.emit(c.hub, "PayoutNotified").withArgs(vaultId, c.receiverAddress, c.wethAddress, payout, false);
    await expect(tx).to.changeTokenBalances(ethers, c.weth, [c.receiverAddress], [payout]);
    await expect(c.hub.connect(c.marketMaker).claimPayout(vaultId)).to.be.revertedWithCustomError(c.hub, "NothingToClaim");
  });
});
