...
contract IvyPremiums {
	...
	function claimable(uint256 id, address holder) external view returns (uint256) {
		Credit storage credit = credits[id][holder];
		Pool storage pool = pools[id];
		if (credit.recorded) return credit.amount;
		if (pool.supply == 0) return 0;
		return Math.mulDiv(pool.amount, IIvyShares(shares).balanceOf(holder, id), pool.supply);
	}

	function _checkpoint(uint256 id, address account, uint256 balance) private {
		Pool storage pool = pools[id];
		if (account == address(0) || pool.supply == 0 || credits[id][account].recorded) return;
		credits[id][account] = Credit(true, Math.mulDiv(pool.amount, balance, pool.supply));
	}
}
