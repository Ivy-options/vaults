...
export async function loadArtifacts(): Promise<Artifacts> {
	return Object.fromEntries(
		await Promise.all(CONTRACTS.map(async name => [name, JSON.parse(await readFile(new URL(artifactPath(name), import.meta.url), "utf8"))])),
	)
}
...
export async function openJournal(signer: Signer, plan: PlanIdentity, journal: Journal): Promise<Required<Pick<Journal, "startBlock" | "steps">>> {
	...
	if (String((await provider.getNetwork()).chainId) !== plan.chainId || (await provider.getBlock(0))?.hash !== plan.genesisHash)
