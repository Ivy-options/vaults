	...
	describe("publishSpot", () => {
		context("with an EOA signer", () => {
			...
			it("rejects an observation dated one second after it is published", async () => {
				...
				await at(c, now + 1n)
				await expect(priceFeed.publishSpot(...spotArgs(future), signature)).to.be.revertedWithCustomError(priceFeed, "InvalidPrice")
			})

			it("rejects a report signed by anyone but the feed signer", async () => {
				await expect(priceFeed.publishSpot(...spotArgs(report), await sign(priceFeed, report, c.bob))).to.be.revertedWithCustomError(
					priceFeed,
					"BadSignature",
				)
			})
			...
			it("rejects a signature made for another feed", async () => {
				...
				await expect(other.publishSpot(...spotArgs(report), await sign(priceFeed, report, c.admin))).to.be.revertedWithCustomError(
					other,
					"BadSignature",
				)
			})
			...
			it("rejects a zero price", async () => {
				await expect(priceFeed.publishSpot(...spotArgs({ ...report, price: 0n }), "0x")).to.be.revertedWithCustomError(priceFeed, "InvalidPrice")
			})
			...
			it("rejects a zero quote", async () => {
				...
				await expect(priceFeed.publishSpot(...spotArgs(zeroQuote), await sign(priceFeed, zeroQuote, c.admin))).to.be.revertedWithCustomError(
					priceFeed,
					"InvalidPrice",
				)
			})
			...
			it("rejects a report whose underlying and quote are the same token", async () => {
				...
				await expect(priceFeed.publishSpot(...spotArgs(sameToken), await sign(priceFeed, sameToken, c.admin))).to.be.revertedWithCustomError(
					priceFeed,
					"InvalidPrice",
				)
			})
			...
			context("with a report valid for 100 seconds", () => {
				...
				it("rejects the report one second after validUntil", async () => {
					...
					await expect(priceFeed.publishSpot(...spotArgs(report), signature)).to.be.revertedWithCustomError(priceFeed, "BidExpired")
				})
			})
		})
		...
		context("with a contract signer", () => {
			...
			context("after a first report", () => {
				...
				it("rejects a replay of the same observation", async () => {
					await expect(priceFeed.publishSpot(...spotArgs(report), signature)).to.be.revertedWithCustomError(priceFeed, "InvalidPrice")
				})
			})
		})
	})
