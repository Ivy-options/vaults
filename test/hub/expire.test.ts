...
describe("expire", () => {
	...
	context("before activation", () => {
		...
		it("reverts with WrongPhase", async () => {
			...
			await expect(c.hub.expire(vaultId)).to.be.revertedWithCustomError(c.hub, "WrongPhase").withArgs(Phase.Live, Phase.Open)
		})
	})
	...
	context("physical call", () => {
		...
		it("is due once the exercise window after expiry closes", async () => {
			expect(await c.hub.expirationTimeOf(v.vaultId)).to.equal(v.bid.expiry + EXERCISE_WINDOW)
		})

		it("reverts before it is due", async () => {
		...
		context("after the exercise window", () => {
			...
			it("lets anyone settle and leaves the collateral and premium to the LPs", async () => {
				await expect(c.hub.connect(c.bob).expire(v.vaultId)).to.emit(c.hub, "Settled").withArgs(v.vaultId, 0n, weth(10), 0n)
			...
			it("does not report a physical fallback lapse", async () => {
				await expect(c.hub.expire(v.vaultId)).to.not.emit(c.hub, "PhysicalFallbackExpired")
			})
		...
		context("after a partial exercise", () => {
			...
			it("settles with the exercised notional", async () => {
				await expect(c.hub.expire(v.vaultId)).to.emit(c.hub, "Settled").withArgs(v.vaultId, weth(4), weth(10), 0n)
			})
		})
	})
	...
	context("cash European call", () => {
		...
		it("waits for the publication and exercise windows while no expiry report exists", async () => {
			expect(await c.hub.expirationTimeOf(v.vaultId)).to.equal(v.bid.expiry + EXPIRY_PRICE_PUBLICATION_WINDOW + EXERCISE_WINDOW)
		})
		...
		context("with an in-the-money expiry report", () => {
			...
			it("is due at the option expiry", async () => {
			...
			it("lets anyone settle and reserves the payout for the market maker", async () => {
				await expect(c.hub.connect(c.alice).expire(v.vaultId)).to.emit(c.hub, "Settled").withArgs(v.vaultId, weth(10), weth(10), CALL_PAYOUT_ALL)
		...
		context("with an out-of-the-money expiry report", () => {
			...
			it("settles without reserving a payout", async () => {
				await expect(c.hub.expire(v.vaultId)).to.emit(c.hub, "Settled").withArgs(v.vaultId, weth(10), weth(10), 0n)
			})
	...
	context("cash European put", () => {
		...
		it("reserves the in-the-money payout in the quote token", async () => {
			await expect(c.hub.expire(v.vaultId)).to.emit(c.hub, "Settled").withArgs(v.vaultId, weth(10), weth(10), usdc(3000))
	...
	context("cash American call after a partial exercise", () => {
		...
		it("settles the unexercised remainder at the expiry report", async () => {
			await expect(c.hub.expire(v.vaultId)).to.emit(c.hub, "Settled").withArgs(v.vaultId, weth(10), weth(10), CALL_PAYOUT_SIX)
		})
	})
