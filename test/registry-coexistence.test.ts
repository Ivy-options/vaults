...
describe("immutable releases coexist", () => {
  it("settles and pays old positions after recommending another Hub with the same local vault ID", async () => {
    ...
    await expect(first.hub.connect(first.marketMaker).exercise(1, 4n * WETH_UNIT)).to.changeTokenBalances(
      ethers,
      first.weth,
    ...
    await expect(first.hub.connect(first.alice).claimPremium(1)).to.changeTokenBalances(
      ...
      first.usdc,
      [first.alice, oldPosition.vaultAddress],
      [1_000n * USDC_UNIT, -1_000n * USDC_UNIT],
    );
    ...
    await expect(second.hub.connect(second.alice).claim(1, 10n * WETH_UNIT)).to.changeTokenBalances(
      ethers,
      second.weth,
      [second.alice, newPosition.vaultAddress],
      [10n * WETH_UNIT, -10n * WETH_UNIT],
    );
