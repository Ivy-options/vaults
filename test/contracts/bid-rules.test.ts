  ...
  describe("validateConfig", () => {
    ...
    describe("PairLimits", () => {
      it("accepts exactly one entry per pair", async () => {
        expect(
          await rules.validateConfig(RuleKind.PairLimits, CALL, PREMIUM_IN_QUOTE, encodePairLimits([[QUOTE, 0n, 0n]])),
    ...
    describe("SpotBand", () => {
      it("accepts a deployed feed, a positive max age and a call band within 100%", async () => {
        expect(
          await rules.validateConfig(RuleKind.SpotBand, CALL, PREMIUM_IN_QUOTE, encodeSpotBand(feedAddress, 60, 1000)),
        ).to.equal(configAccepted);
      });
      ...
      it("accepts a call band of exactly 100%", async () => {
        expect(
          await rules.validateConfig(RuleKind.SpotBand, CALL, PREMIUM_IN_QUOTE, encodeSpotBand(feedAddress, 60, 10_000)),
        ).to.equal(configAccepted);
      });
      ...
      it("accepts a put band above 100%", async () => {
        expect(
          await rules.validateConfig(RuleKind.SpotBand, PUT, PREMIUM_IN_QUOTE, encodeSpotBand(feedAddress, 60, 20_000)),
        ).to.equal(configAccepted);
      });
    });
    ...
    describe("PremiumFloor", () => {
      context("when every pair pays the premium in the underlying", () => {
        it("accepts a floor without a feed on a call", async () => {
          expect(
            await rules.validateConfig(
              RuleKind.PremiumFloor,
              CALL,
              PREMIUM_IN_UNDERLYING,
              encodePremiumFloor(ZeroAddress, 0, 500),
            ),
          ).to.equal(configAccepted);
        });
        ...
        it("accepts a floor with a deployed feed", async () => {
          expect(
            await rules.validateConfig(
              ...
              CALL,
              PREMIUM_IN_QUOTE,
              encodePremiumFloor(feedAddress, 60, 500),
            ),
          ).to.equal(configAccepted);
        });
        ...
        it("accepts a floor of exactly 100%", async () => {
          expect(
            await rules.validateConfig(
              ...
              CALL,
              PREMIUM_IN_QUOTE,
              encodePremiumFloor(feedAddress, 60, 10_000),
            ),
          ).to.equal(configAccepted);
        });
          ...
          it(`rejects ${floor.name}`, async () => {
            await expect(
              rules.validateConfig(
                ...
                CALL,
                PREMIUM_IN_QUOTE,
                encodePremiumFloor(feedAddress, 60, floor.bps),
              ),
    ...
    describe("SpotBand", () => {
      context("with a spot dated the latest block", () => {
        ...
        it("accepts the spot in the second it is dated", async () => {
          expect(
            await rules.validateBid(RuleKind.SpotBand, CALL_CONTEXT, BID, encodeSpotBand(feedAddress, 60, 1000)),
          ).to.equal(bidAccepted);
        });
      });
      ...
      context("with a spot dated one second after the latest block", () => {
        ...
        it("rejects a spot dated one second after the latest block", async () => {
          await expect(
            rules.validateBid(RuleKind.SpotBand, CALL_CONTEXT, BID, encodeSpotBand(feedAddress, 60, 1000)),
          ).to.be.revertedWithCustomError(rules, "InvalidPrice");
        });
      });
    });
  });
