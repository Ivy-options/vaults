...
describe("platform fees and transferable unpaid premium", () => {
  let c: IvyContext;
  beforeEach(async () => {
    c = await load();
  });
  it("disables every transfer path initially while permitting deposits, claims and burns", async () => {
    const v = await goLive(c);
    expect(await c.hub.transfersEnabled()).eq(false);
    await expect(c.hub.connect(c.alice).setTransfersEnabled(true)).revertedWithCustomError(
      c.hub,
      "AccessControlUnauthorizedAccount",
    );
    await c.shares.connect(c.alice).setApprovalForAll(c.bob.address, true);
    await expect(
      c.shares.connect(c.bob).safeTransferFrom(c.alice.address, c.carol.address, v.vaultId, W, "0x"),
    ).revertedWithCustomError(c.shares, "TransfersDisabled");
    await expect(
      c.shares.connect(c.alice).safeBatchTransferFrom(c.alice.address, c.bob.address, [v.vaultId], [W], "0x"),
    ).revertedWithCustomError(c.shares, "TransfersDisabled");
  ...
  it("moves additive remaining credit proportionally, including after claims and partial burns", async () => {
  ...
  it("authorizes the global rate and keeps the fee within the gross premium", async () => {
    expect(await c.hub.platformFeeBps()).eq(0);
    await expect(c.hub.connect(c.alice).setPlatformFeeBps(100)).revertedWithCustomError(
      c.hub,
      "AccessControlUnauthorizedAccount",
    );
  ...
  for (const [creationRate, activationRate, feeUnits] of [
    [0, 200, 0],
    [200, 500, 20],
    [500, 100, 50],
    [500, 0, 50],
    ...
    it(`keeps the creation rate after the global rate changes from ${creationRate} to ${activationRate} bps`, async () => {
      ...
      await expect(c.hub.connect(c.bidMaster).activate(v.vaultId, bid, signature)).changeTokenBalances(
        connection.ethers,
        c.usdc,
        [c.marketMaker, v.vault],
        [-gross, gross],
      );
  ...
  it("fixes the rate before deposits and applies new defaults only to new vaults", async () => {
  ...
  it("retains the creation rate through a cancelled and reopened auction", async () => {
    await c.hub.setPlatformFeeBps(200);
    const v = await openVault(c);
  ...
  it("charges the fixed vault fee despite later global rate changes", async () => {
    await c.hub.setPlatformFeeBps(200);
    const v = await openVault(c);
    const bid = await makeBid(c, v.vaultId);
    const signature = await signBid(c.marketMaker, c.hubAddress, bid);
  ...
  it("segregates fees from overlapping collateral and net LP premium, and snapshots the recipient", async () => {
    await c.hub.setPlatformFeeBps(200);
    await c.hub.setPlatformTreasury(c.carol.address);
    ...
    await expect(v.vault.connect(c.bob).claimPlatformFee()).changeTokenBalances(
      connection.ethers,
      c.usdc,
      [c.carol],
      [20n * U],
  ...
  it("rounds the fee down and permits the full-rate boundary", async () => {
  ...
  it("rolls back fee allocation, nonce and premium on short funding", async () => {
    await c.hub.setPlatformFeeBps(200);
    const v = await openVault(c);
    const bid = await makeBid(c, v.vaultId);
    const signature = await signBid(c.marketMaker, c.hubAddress, bid);
  ...
  it("retains platform fees through separately funded unwind with buyer reserves", async () => {
    await c.hub.setPlatformFeeBps(200);
    ...
    await c.usdc.mint(c.alice.address, a.refund);
