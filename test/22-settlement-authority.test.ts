...
describe("Hub settlement authority", function () {
  it("requires current publisher authority and retains finalized prices after rotation", async function () {
    ...
    await expect(hub.connect(c.bob).publishExpiry(c.wethAddress, c.usdcAddress, now, 3000, now + 100n))
      .revertedWithCustomError(hub, "AccessControlUnauthorizedAccount");
    await hub.publishExpiry(c.wethAddress, c.usdcAddress, now, 3000, now + 100n);
    ...
    await expect(hub.publishExercisePrice(c.wethAddress, c.usdcAddress, 4000, now, now + 100n))
      .revertedWithCustomError(hub, "AccessControlUnauthorizedAccount");
    await expect(hub.connect(c.bob).publishExpiry(c.wethAddress, c.usdcAddress, now, 4000, now + 100n))
  ...
  it("enforces age and report-validity boundaries independently of indicative prices", async function () {
    ...
    await c.hub.publishExercisePrice(c.wethAddress, c.usdcAddress, 4000n * U, now - 4000n, now + 1000n);
    ...
    await c.hub.publishExercisePrice(c.wethAddress, c.usdcAddress, 4000n * U, ageBoundary - 3600n, ageBoundary + 100n);
    ...
    await c.hub.publishExercisePrice(c.wethAddress, c.usdcAddress, 4000n * U, now, validityBoundary);
  ...
  it("locks missing expiry obligations until late recovery while keeping premium, fee and buyer reserves separate", async function () {
    ...
    const now = BigInt(await c.networkHelpers.time.latest());
    await c.hub.connect(c.carol).publishExpiry(c.wethAddress, c.usdcAddress, v.bid.expiry, 2700n * U, now + 100n);
  ...
  it("validates reports and finalizes each exact pair/expiry once at the inclusive publication boundary", async function () {
    const c = await deployIvy(await network.create());
    const hub = c.hub;
    let now = BigInt(await c.networkHelpers.time.latest());
    for (const [underlying, quote, price] of [
      [ZeroAddress, c.usdcAddress, 1], [c.wethAddress, ZeroAddress, 1],
      [c.wethAddress, c.wethAddress, 1], [c.wethAddress, c.usdcAddress, 0],
    ] as const) {
      await expect(hub.publishExercisePrice(underlying, quote, price, now, now + 1000n)).revertedWithCustomError(hub, "InvalidPrice");
      await expect(hub.publishExpiry(underlying, quote, now, price, now + 1000n)).revertedWithCustomError(hub, "InvalidPrice");
    }
    await expect(hub.publishExercisePrice(c.wethAddress, c.usdcAddress, 1, 0, now + 1000n)).revertedWithCustomError(hub, "InvalidPrice");
    await expect(hub.publishExercisePrice(c.wethAddress, c.usdcAddress, 1, now + 1000n, now + 2000n)).revertedWithCustomError(hub, "InvalidPrice");
    await expect(hub.publishExpiry(c.wethAddress, c.usdcAddress, now + 1000n, 1, now + 2000n)).revertedWithCustomError(hub, "ExpirationNotReached");
    await expect(hub.publishExpiry(c.wethAddress, c.usdcAddress, 0, 1, now + 2000n)).revertedWithCustomError(hub, "ExpirationNotReached");
    now = BigInt(await c.networkHelpers.time.latest());
    const publicationBoundary = now + 1n;
    await at(c, publicationBoundary);
    await expect(hub.publishExercisePrice(c.wethAddress, c.usdcAddress, 4000, publicationBoundary, publicationBoundary))
      .emit(hub, "ExercisePricePublished").withArgs(c.wethAddress, c.usdcAddress, 4000, publicationBoundary, publicationBoundary);
    await expect(hub.publishExercisePrice(c.wethAddress, c.usdcAddress, 5000, publicationBoundary, now + 100n)).revertedWithCustomError(hub, "InvalidPrice");
    await expect(hub.publishExercisePrice(c.wethAddress, c.usdcAddress, 5000, now, now + 100n)).revertedWithCustomError(hub, "InvalidPrice");
    await expect(hub.publishExercisePrice(c.wethAddress, c.usdcAddress, 5000, now + 1n, now)).revertedWithCustomError(hub, "BidExpired");
    await expect(hub.publishExpiry(c.wethAddress, c.usdcAddress, now, 5000, now)).revertedWithCustomError(hub, "BidExpired");
    expect(await hub.exercisePrice(c.wethAddress, c.daiAddress)).deep.equal([0n, 0n, 0n]);
    const boundary = BigInt(await c.networkHelpers.time.latest()) + 1n;
    await at(c, boundary);
    await expect(hub.publishExpiry(c.wethAddress, c.usdcAddress, boundary, 3000, boundary))
      .emit(hub, "ExpiryPublished").withArgs(c.wethAddress, c.usdcAddress, boundary, 3000, boundary);
    await expect(hub.settlementPrice(c.wethAddress, c.daiAddress, boundary)).revertedWithCustomError(hub, "ReportUnavailable");
    await expect(hub.settlementPrice(c.wethAddress, c.usdcAddress, boundary + 1n)).revertedWithCustomError(hub, "ReportUnavailable");
  });

  it("uses the selected pair only and retains completed early payments after publisher rotation", async function () {
    const c = await deployIvy(await network.create());
    const v = await goLive(c, { withFeed: true }, { settlement: SettlementType.Cash });
    let now = BigInt(await c.networkHelpers.time.latest());
    await c.hub.publishExercisePrice(c.wethAddress, c.daiAddress, 6000n * U, now, now + 100n);
    await expect(c.hub.connect(c.marketMaker).exercise(v.vaultId, W)).revertedWithCustomError(c.hub, "InvalidPrice");
    now = BigInt(await c.networkHelpers.time.latest());
    await c.hub.publishExercisePrice(c.wethAddress, c.usdcAddress, 4000n * U, now, now + 100n);
    const role = await c.hub.SETTLEMENT_PRICE_PUBLISHER_ROLE();
    await c.hub.revokeRole(role, c.admin.address);
    await expect(c.hub.connect(c.marketMaker).exercise(v.vaultId, W)).changeTokenBalance(c.ethers, c.weth, c.marketMaker, W / 4n);
    await c.hub.grantRole(role, c.carol.address);
    now = BigInt(await c.networkHelpers.time.latest());
    await c.hub.connect(c.carol).publishExercisePrice(c.wethAddress, c.usdcAddress, 6000n * U, now, now + 100n);
  ...
  it("shares one exercise observation and exact expiry finalization across matching vaults", async function () {
    const c = await deployIvy(await network.create());
    ...
    await c.hub.publishExpiry(c.wethAddress, c.usdcAddress, c.defaultExpiry, 6000n * U, c.defaultExpiry + 100n);
    for (const v of [first, second]) {
      await c.hub.expire(v.vaultId);
      expect((await c.hub.stateOf(v.vaultId)).pendingPayout).equal(9n * W / 2n);
    }
    await expect(c.hub.publishExpiry(c.wethAddress, c.usdcAddress, c.defaultExpiry, 7000n * U, c.defaultExpiry + 100n))
      .revertedWithCustomError(c.hub, "ReportFinalized");
  });
  ...
  it("switches from a still-valid American observation to the exact expiry report at the expiry second", async function () {
    ...
    await c.hub.publishExercisePrice(c.wethAddress, c.usdcAddress, 4000n * U, v.bid.expiry - 2n, v.bid.expiry + 100n);
    ...
    await c.hub.publishExpiry(c.wethAddress, c.usdcAddress, v.bid.expiry, 6000n * U, v.bid.expiry + 100n);
    await expect(c.hub.connect(c.marketMaker).exercise(v.vaultId, W)).changeTokenBalance(c.ethers, c.weth, c.marketMaker, W / 2n);
    await c.hub.expire(v.vaultId);
