...
describe("exercise policy and expiration", () => {
  let c: IvyContext;
  beforeEach(async () => {
    c = await load();
  });
    ...
    for (const style of [ExerciseStyle.American, ExerciseStyle.European]) {
      for (const settlement of [SettlementType.Physical, SettlementType.Cash]) {
        for (const isCall of [false, true]) {
          it(`enforces partial=${allowPartialExercise}, style=${style}, settlement=${settlement}, call=${isCall}`, async () => {
            const v = await goLive(
              c,
              { isCall, withFeed: settlement === SettlementType.Cash, terms: { allowPartialExercise } },
              { style, settlement },
            );
            expect((await c.hub.termsOf(v.vaultId)).allowPartialExercise).eq(allowPartialExercise);
            if (settlement === SettlementType.Physical)
            ...
            if (!allowPartialExercise) {
              await expect(c.hub.connect(c.marketMaker).exercise(v.vaultId, 4n * W)).revertedWithCustomError(
                c.hub,
  ...
  for (const allowPartialExercise of [false, true])
    it(`keeps partial policy ${allowPartialExercise} and exposes no tightening`, async () => {
  ...
  for (const style of [ExerciseStyle.American, ExerciseStyle.European])
    it(`cash style ${style}: expiry exercise pays only chosen units, expiration reserves the rest`, async () => {
      const v = await goLive(
        c,
      ...
      await expect(c.hub.connect(c.bob).exercise(v.vaultId, 4n * W)).changeTokenBalances(
        ethers,
        c.usdc,
      ...
      await expect(c.hub.connect(c.bob).claimPayout(v.vaultId)).changeTokenBalances(
        ethers,
        c.usdc,
        [c.carol, v.vaultAddress],
        [1800n * U, -1800n * U],
  ...
  it("full-only out-of-money cash stays live before expiry and permissionlessly expires with no payout", async () => {
    const v = await goLive(
      c,
    ...
    await expect(c.hub.connect(c.marketMaker).exercise(v.vaultId, 10n * W)).revertedWithCustomError(
      c.hub,
      "NothingToExercise",
    );
  ...
  it("a blocked cash recipient cannot stop expiration or erase their payout", async () => {
    const v = await goLive(
      c,
      { withFeed: true, terms: { allowPartialExercise: false } },
      { style: ExerciseStyle.European, settlement: SettlementType.Cash, recipient: c.carol.address },
    );
