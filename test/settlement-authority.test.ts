...
describe("Hub settlement authority", () => {
  let c: IvyContext;
  beforeEach(async () => {
    c = await load();
  });

  it("requires current publisher authority and retains finalized prices after rotation", async () => {
    const hub = c.hub;
    ...
    await expect(hub.connect(c.bob).publishExpiry(v.vaultId, 3000, now + 100n)).revertedWithCustomError(
      hub,
      "AccessControlUnauthorizedAccount",
    );
    await hub.publishExpiry(v.vaultId, 3000, now + 100n);
    await expect(hub.connect(c.bob).grantRole(role, c.bob.address)).revertedWithCustomError(
      hub,
      "AccessControlUnauthorizedAccount",
    );
    await hub.grantRole(role, c.bob.address);
    await hub.revokeRole(role, c.admin.address);
    await expect(hub.publishExercisePrice(v.vaultId, 4000, now, now + 100n)).revertedWithCustomError(
      hub,
      "AccessControlUnauthorizedAccount",
    );
    await expect(hub.connect(c.bob).publishExpiry(v.vaultId, 4000, now + 100n)).revertedWithCustomError(
      hub,
      "ReportFinalized",
    );
  ...
  it("requires and preserves a cash observation age limit without a source-contract binding", async () => {
    await expect(c.hub.createVault(callTerms(c, { allowedSettlement: 1 }), callPairs(c), [])).revertedWithCustomError(
      c.hub,
      "CashSettlementNeedsMaxPriceAge",
    );
    await c.hub
      .connect(c.alice)
      .createVault(callTerms(c, { allowedSettlement: 1, maxSettlementPriceAge: 60 }), callPairs(c), []);
    expect((await c.hub.termsOf(1)).maxSettlementPriceAge).equal(60);
    expect(await c.hub.rulesOf(1)).deep.equal([]);
  });

  it("enforces age and report-validity boundaries independently of indicative prices", async () => {
    const v = await goLive(c, { withFeed: true }, { settlement: SettlementType.Cash });
  ...
  it("allows publisher recovery within the publication window while keeping premium, fee and buyer reserves separate", async () => {
  ...
  it("validates per-vault reports and finalizes once at the inclusive publication boundary", async () => {
    const hub = c.hub;
    const v = await goLive(c, { withFeed: true }, { settlement: SettlementType.Cash });
    let now = BigInt(await c.networkHelpers.time.latest());
    ...
    await expect(hub.publishExercisePrice(v.vaultId, 1, now + 1000n, now + 2000n)).revertedWithCustomError(
      hub,
      "InvalidPrice",
    );
    await expect(hub.publishExpiry(v.vaultId, 1, v.bid.expiry + 2000n)).revertedWithCustomError(
      hub,
    ...
    await expect(hub.publishExercisePrice(v.vaultId, 4000, publicationBoundary, publicationBoundary))
      .emit(hub, "ExercisePricePublished")
      .withArgs(v.vaultId, c.wethAddress, c.usdcAddress, 4000, publicationBoundary, publicationBoundary);
    await expect(hub.publishExercisePrice(v.vaultId, 5000, publicationBoundary, now + 100n)).revertedWithCustomError(
      hub,
      "InvalidPrice",
    );
    await expect(hub.publishExercisePrice(v.vaultId, 5000, now, now + 100n)).revertedWithCustomError(
      hub,
      "InvalidPrice",
    );
    ...
    await expect(hub.publishExpiry(v.vaultId, 3000, v.bid.expiry))
      .emit(hub, "ExpiryPublished")
      .withArgs(v.vaultId, c.wethAddress, c.usdcAddress, v.bid.expiry, 3000, v.bid.expiry);
    await expect(hub.publishExpiry(v.vaultId, 4000, v.bid.expiry + 100n)).revertedWithCustomError(
      hub,
      "ReportFinalized",
    );
  });

  it("retains completed early payments after publisher rotation", async () => {
    const v = await goLive(c, { withFeed: true }, { settlement: SettlementType.Cash });
    let now = BigInt(await c.networkHelpers.time.latest());
    await expect(c.hub.connect(c.marketMaker).exercise(v.vaultId, W)).revertedWithCustomError(c.hub, "InvalidPrice");
    now = BigInt(await c.networkHelpers.time.latest());
  ...
  it("rejects unknown, unactivated, physical and finalized vault publication", async () => {
    const now = BigInt(await c.networkHelpers.time.latest());
    for (const vaultId of [0n, 999n]) {
      await expect(c.hub.publishExercisePrice(vaultId, 4000, now, now + 1000n)).revertedWithCustomError(
        c.hub,
        "UnknownVault",
      );
    ...
    const opened = await createVaultAs(
      c,
      c.alice,
      callTerms(c, { allowedSettlement: SettlementType.Cash, maxSettlementPriceAge: 3600 }),
      callPairs(c),
    );
    ...
    await expect(c.hub.publishExercisePrice(cash.vaultId, 4000, latest + 1n, latest + 100n)).revertedWithCustomError(
      c.hub,
      "WrongPhase",
    );
  ...
  it("switches from a still-valid American observation to the exact expiry report at the expiry second", async () => {
