...
describe('cross-module callbacks and reserve invariants',function(){
 ...
 for(const isCall of [true,false]) for(const cash of [true,false]) it(`stateful conservation: ${isCall?'call':'put'}, ${cash?'cash':'physical'}`,async()=>{
  ...
  if(cash){await c.feed.setSettlementPrice(c.wethAddress,c.usdcAddress,v.bid.expiry,isCall?3300n*U:2700n*U);await at(c,v.bid.expiry);await c.hub.settle(v.vaultId);}
  else {if(!isCall)await fund(c,c.weth,c.marketMaker,v.vaultAddress,4n*W);await c.hub.connect(c.marketMaker).exercise(v.vaultId,4n*W);await at(c,v.bid.expiry+3600n);await c.hub.settle(v.vaultId);}
