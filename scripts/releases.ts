...
import { CONTRACTS, buildDeploymentPlan, json, planHash, rpc, verifyCreation, verifyBindings } from "./deployment.ts";
import type { Artifacts, DeploymentPlan, Journal } from "./deployment.ts";

export interface ReleaseBundle {
  format: number;
  interfaceFormat: string;
  manifest: DeploymentPlan;
  journal: Journal;
  artifacts: Artifacts;
}
...
export async function verifyRelease(
  ...
  if (bundle.format !== 1 || bundle.interfaceFormat !== RELEASE_FORMAT || bundle.manifest?.version !== 7)
  ...
  if (!manifest.addresses?.IvyBidRules || !manifest.steps?.some((s) => s.name === "IvyBidRules"))
    throw new Error("Release manifest lacks IvyBidRules");
  if (
    String((await provider.getNetwork()).chainId) !== manifest.chainId ||
    (await provider.getBlock(0))?.hash !== manifest.genesisHash
  )
  ...
  for (const name of CONTRACTS) {
    if (
      ...
      json(artifacts[name].abi) !== json(supportedArtifacts[name].abi)
    )
      throw new Error(`Unsupported release interface: ${name}`);
  }
  ...
  for (const step of manifest.steps) {
    ...
    if (keccak256(historical) !== hash || entry.runtimeHash !== hash)
      throw new Error(`Runtime history mismatch: ${step.name}`);
  }
  ...
  for (const [contractName, name, version] of [
    ...
    if (
      ...
      domain[1] !== name ||
      domain[2] !== version ||
      String(domain[3]) !== manifest.chainId ||
      getAddress(domain[4]) !== getAddress(address) ||
      domain[6].length !== 0
    )
      throw new Error(`Unsupported signing domain: ${contractName}`);
  }
  return {
    hub: getAddress(manifest.addresses.IvyVaultsHub),
    addresses: manifest.addresses,
    chainId: manifest.chainId,
    artifacts,
    manifestHash: releaseHash(bundle),
  };
}
...
export async function resolveRelease(
  ...
  if (releaseId === undefined) {
    ...
    releaseId = (await registry.recommendedVersion()) as bigint;
  ...
  if ((await registry.manifestHashOf(releaseId)) !== releaseHash(bundle))
    throw new Error("Registered manifest hash mismatch");
