...
contract CallbackToken is ERC20 {
    ...
    function approveSelf(address spender, uint256 amount) external { _approve(address(this), spender, amount); }
    function arm(address target_, bytes calldata data_) external { callbackTarget = target_; data = data_; enabled = true; callbackSucceeded = false; }
    function disarm() external { enabled = false; }
