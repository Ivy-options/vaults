...
- `deployment.json`: read-only `prepare-deployment` input. Save its output as deployment-plan.json. It predicts nine deployments, including the two linked libraries before the hub, the indicative feed and the authoritative settlement feed. Specify settlement admin and publisher separately; settlementMethodology references the approved per-pair methodology artifact.
...
- `settlement-exercise.json`: `publish-settlement-exercise` input for a direct authorized publisher transaction. Its `validUntil` remains enforced at exercise, alongside the vault’s maximum observation age.
- `settlement-expiry.json`: `publish-settlement-expiry` input for the exact expiry historical calculation. Publication is irreversible.
- `settlement-role.json`: `grant-settlement-publisher` or `revoke-settlement-publisher` input targeting the authoritative feed, sent by its admin.
...
For cash-enabled vaults, set `terms.settlementPriceFeed` to the new authoritative feed, choose a positive `terms.maxSettlementPriceAge`, and include `settlementMethodology` in the request. These terms are fixed at creation. `priceFeed` and `maxPriceAge` only govern optional activation checks; publishing signed spot or expiry reports there cannot resolve a missing authoritative report.
