...
describe("cash settlement physical fallback", () => {
  let c: IvyContext;
  beforeEach(async () => {
    c = await load();
  });

  it("fixes publication and physical windows before participation and rejects unusable cash timings", async () => {
    ...
    for (const allowedSettlement of [SettlementPolicy.Cash, SettlementPolicy.Either]) {
      await expect(c.hub.createVault({ ...terms, allowedSettlement }, callPairs(c), [])).revertedWithCustomError(
        c.hub,
  ...
  it("lets an unrelated caller release LP collateral after an entirely unattended publisher outage", async () => {
    ...
    await expect(c.hub.connect(c.carol).expire(v.vaultId))
      .emit(c.hub, "Settled")
      .withArgs(v.vaultId, 0n, 10n * W, 0n);
    expect(await v.vault.buyerReserved(c.wethAddress)).eq(0n);
    await expect(c.hub.connect(c.alice).claim(v.vaultId, 10n * W)).changeTokenBalance(
      c.ethers,
      c.weth,
      c.alice,
      10n * W,
  ...
  it("opens explicit physical call exercise at the fixed deadline and closes late publication", async () => {
    const v = await goLive(c, { withFeed: true }, { settlement: SettlementType.Cash });
    const deadline = v.bid.expiry + P;
    await at(c, deadline - 1n);
    await expect(c.hub.connect(c.marketMaker).exercisePhysicalFallback(v.vaultId, W)).revertedWithCustomError(
      c.hub,
    ...
    await expect(tx)
      ...
      .withArgs(v.vaultId, W, 3000n * U, W);
    ...
    await expect(c.hub.publishExpiry(v.vaultId, 6000n * U, deadline + 100n)).revertedWithCustomError(
      c.hub,
      "ExpiryPricePublicationClosed",
    );
    await expect(c.hub.connect(c.marketMaker).exercise(v.vaultId, W)).revertedWithCustomError(
      c.hub,
      "ReportUnavailable",
    );
  ...
  it("reports effective routes and fixed deadlines without a fallback activation transaction", async () => {
    const v = await goLive(c, { withFeed: true }, { settlement: SettlementType.Cash });
    ...
    await expect(
      c.hub.connect(c.marketMaker).exercisePhysicalFallback.staticCall(v.vaultId, W),
    ).revertedWithCustomError(c.hub, "ExerciseWindowClosed");
    await expect(c.hub.expire(v.vaultId))
      .emit(c.hub, "PhysicalFallbackExpired")
      .withArgs(v.vaultId, 10n * W);
  ...
  for (const style of [ExerciseStyle.American, ExerciseStyle.European]) {
    for (const isCall of [true, false]) {
      ...
      it(`${label}: partial exercise at F - 1 and permissionless lapse at F preserve LP proceeds`, async () => {
        ...
        await expect(tx).changeTokenBalance(
          c.ethers,
          isCall ? c.weth : c.usdc,
          c.marketMaker,
          isCall ? 4n * W : 12000n * U,
        );
        await at(c, f);
        await expect(c.hub.connect(c.carol).expire(v.vaultId))
          .emit(c.hub, "PhysicalFallbackExpired")
          .withArgs(v.vaultId, 6n * W)
          .and.emit(c.hub, "Settled")
          .withArgs(v.vaultId, 4n * W, 10n * W, 0n);
        ...
        await expect(c.hub.connect(c.marketMaker).claimPayout(v.vaultId)).revertedWithCustomError(
          c.hub,
          "NothingToClaim",
        );
      });
      ...
      for (const inTheMoney of [true, false]) {
        it(`${label}: a timely ${inTheMoney ? "ITM" : "OTM"} final price stays authoritative after F`, async () => {
          const v = await goLive(c, { isCall, withFeed: true }, { settlement: SettlementType.Cash, style });
          const d = v.bid.expiry + P;
          const f = d + EXERCISE_WINDOW;
          ...
          await expect(c.hub.expire(v.vaultId))
            .emit(c.hub, "Settled")
            .withArgs(v.vaultId, 10n * W, 10n * W, payout);
  ...
  it("rejects final publication at D even when publication is the first interaction", async () => {
    const v = await goLive(c, { withFeed: true }, { settlement: SettlementType.Cash });
    const d = v.bid.expiry + P;
    await at(c, d);
    await expect(c.hub.publishExpiry(v.vaultId, 6000n * U, d + 100n)).revertedWithCustomError(
      c.hub,
      "ExpiryPricePublicationClosed",
    );
    await expect(c.hub.connect(c.marketMaker).exercisePhysicalFallback(v.vaultId, W)).emit(
      c.hub,
      "PhysicalFallbackExercised",
    );
  ...
  for (const publishFirst of [true, false]) {
    it(`publication and fallback mined together at D are exclusive, ${publishFirst ? "publication" : "fallback"} first`, async () => {
      const v = await goLive(c, { withFeed: true }, { settlement: SettlementType.Cash });
      const d = v.bid.expiry + P;
      ...
      try {
        ...
        const exercise = () =>
          c.hub.connect(c.marketMaker).exercisePhysicalFallback(v.vaultId, W, { gasLimit: 500000 });
  ...
  it("never enables early fallback for missing or stale American observations or early European exercise", async () => {
    const american = await goLive(c, { withFeed: true }, { settlement: SettlementType.Cash });
    await expect(c.hub.connect(c.marketMaker).exercise(american.vaultId, W)).revertedWithCustomError(
      c.hub,
      "InvalidPrice",
    );
    await expect(c.hub.connect(c.marketMaker).exercisePhysicalFallback(american.vaultId, W)).revertedWithCustomError(
      c.hub,
      "PhysicalFallbackUnavailable",
    );
    ...
    const european = await goLive(
      c,
      { withFeed: true },
      { settlement: SettlementType.Cash, style: ExerciseStyle.European },
    );
    await expect(c.hub.connect(c.marketMaker).exercise(european.vaultId, W)).revertedWithCustomError(
      c.hub,
      "ExerciseNotOpenYet",
    );
    await expect(c.hub.connect(c.marketMaker).exercisePhysicalFallback(european.vaultId, W)).revertedWithCustomError(
      c.hub,
      "PhysicalFallbackUnavailable",
    );
  ...
  it("uses only the remainder after American cash exercise and finalizes immediately on full fallback", async () => {
    const v = await goLive(c, { withFeed: true }, { settlement: SettlementType.Cash });
    await setExercisePrice(c, v.vaultId, 6000n * U);
    await expect(c.hub.connect(c.marketMaker).exercise(v.vaultId, 4n * W)).changeTokenBalance(
      c.ethers,
      c.weth,
      c.marketMaker,
      2n * W,
  ...
  it("honors all-or-nothing exercise and existing executor and recipient authorization", async () => {
    const v = await goLive(
      c,
      { isCall: false, withFeed: true, terms: { allowPartialExercise: false } },
    ...
    await at(c, v.bid.expiry + P);
    await expect(c.hub.connect(c.alice).exercisePhysicalFallback(v.vaultId, 10n * W)).revertedWithCustomError(
      c.hub,
      "NotExecutor",
    );
    await expect(c.hub.connect(c.bob).exercisePhysicalFallback(v.vaultId, W)).revertedWithCustomError(
      c.hub,
      "PartialExerciseNotAllowed",
    );
    await expect(c.hub.connect(c.bob).exercisePhysicalFallback(v.vaultId, 0)).revertedWithCustomError(
      c.hub,
      "ZeroAmount",
    );
  ...
  it("keeps windows and settlement available through paused admissions, settings changes and publisher removal", async () => {
    const v = await goLive(c, { withFeed: true }, { settlement: SettlementType.Cash });
    const d = v.bid.expiry + P;
    const f = d + EXERCISE_WINDOW;
    ...
    await at(c, d);
    await expect(c.hub.connect(c.bob).publishExpiry(v.vaultId, 6000n * U, f)).revertedWithCustomError(
      c.hub,
      "ExpiryPricePublicationClosed",
    );
  ...
  for (const isCall of [true, false]) {
    it(`${isCall ? "call" : "put"}: failed physical transfers preserve funds, notional and reserves atomically`, async () => {
      const v = await goLive(
        c,
        { isCall, withFeed: true },
        {
          settlement: SettlementType.Cash,
      ...
      await expect(c.hub.connect(c.bob).exercisePhysicalFallback(v.vaultId, W))
        .revertedWithCustomError(c.hub, "ShortReceived")
    ...
    it(`${isCall ? "call" : "put"}: preserves strike rounding across 18-decimal underlying and 6-decimal quote`, async () => {
      ...
      await expect(c.hub.connect(c.marketMaker).exercisePhysicalFallback(v.vaultId, 333333334n))
        .emit(c.hub, "PhysicalFallbackExercised")
        .withArgs(v.vaultId, 333333334n, isCall ? 2n : 333333334n, isCall ? 333333334n : 1n);
    });

    it(`${isCall ? "call" : "put"}: mixed LP claims exclude premiums, fees and recoverable unwind contributions`, async () => {
      await c.hub.setPlatformFeeBps(200);
      const aliceShares = isCall ? 6n * W : 18000n * U;
      const bobShares = isCall ? 4n * W : 12000n * U;
      const v = await goLive(
        c,
        { isCall, withFeed: true, deposit: aliceShares, extraDeposits: [{ signer: c.bob, amount: bobShares }] },
        { settlement: SettlementType.Cash },
      );
      const f = v.bid.expiry + P + EXERCISE_WINDOW;
      ...
      await expect(aliceClaim).changeTokenBalance(
        c.ethers,
        c.weth,
        c.alice,
        isCall ? 3600000000000000000n : 2400000000000000000n,
      );
      ...
      await expect(c.hub.connect(c.alice).claimPremium(v.vaultId)).changeTokenBalance(
        c.ethers,
        c.usdc,
        c.alice,
        588n * U,
      ...
      await expect(c.hub.connect(c.alice).withdrawUnwindContribution(v.vaultId, agreement.nonce)).changeTokenBalance(
        c.ethers,
        c.usdc,
        c.alice,
        60n * U,
      ...
      await expect(c.hub.connect(c.alice).claim(v.vaultId, aliceShares)).revertedWithCustomError(
        c.hub,
        "InsufficientShares",
      );
    });
  ...
  it("delivers a full fallback payout and finalizes before notifying the chosen recipient", async () => {
    ...
    await expect(c.hub.connect(c.marketMaker).exercisePhysicalFallback(v.vaultId, 10n * W))
      .emit(c.hub, "PayoutNotified")
      .withArgs(v.vaultId, receiverAddress, c.wethAddress, 10n * W, true);
  ...
  it("finalizes at F without calling an unresponsive recipient", async () => {
    const receiver = await c.ethers.deployContract("PayoutReceiver", [c.hubAddress]);
    await receiver.setMode(4);
    const v = await goLive(
      c,
      { withFeed: true },
      { settlement: SettlementType.Cash, recipient: await receiver.getAddress() },
    );
  ...
  for (const finalization of ["cash exercise", "unwind"] as const) {
    it(`cannot revive a position finalized by ${finalization}`, async () => {
      const v = await goLive(c, { withFeed: true }, { settlement: SettlementType.Cash });
      ...
      await expect(c.hub.connect(c.alice).claim(v.vaultId, 10n * W)).revertedWithCustomError(
        c.hub,
        "InsufficientShares",
      );
    });
  ...
  it("retains original physical timing and rejects the cash-specific fallback entrypoint", async () => {
