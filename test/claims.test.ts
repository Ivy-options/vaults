...
describe("claim", () => {
  ...
  async function expiredCall(c: IvyContext) {
    const live = await goLive(c, {
      deposit: 6n * WETH_UNIT,
      extraDeposits: [{ signer: c.bob, amount: 4n * WETH_UNIT }],
    });
    await at(c, live.bid.expiry + EXERCISE_WINDOW + 1n);
    await c.hub.expire(live.vaultId);
    return live;
  }

  it("expired physical call: collateral splits while unpaid premium stays reserved", async () => {
    const { hub, weth, usdc, alice, bob } = c;
    const { vaultId, vaultAddress } = await expiredCall(c);

    const txA = hub.connect(alice).claim(vaultId, 6n * WETH_UNIT);
    await expect(txA)
      .to.emit(hub, "Claimed")
      .withArgs(vaultId, alice.address, 6n * WETH_UNIT);
  ...
  it("fully exercised physical call: LPs receive quote, premium remains separately claimable", async () => {
    const { hub, weth, usdc, alice, bob, marketMaker } = c;
    const { vaultId, vaultAddress } = await goLive(c, {
      deposit: 6n * WETH_UNIT,
      extraDeposits: [{ signer: bob, amount: 4n * WETH_UNIT }],
    });
    await fund(c, usdc, marketMaker, vaultAddress, 30_000n * USDC_UNIT);
    ...
    await expect(hub.connect(bob).claim(vaultId, 4n * WETH_UNIT)).to.changeTokenBalances(
      ethers,
      usdc,
      [bob],
      [12_000n * USDC_UNIT],
  ...
  it("partially exercised put: LPs receive a mixed pot", async () => {
    const { hub, weth, usdc, alice, bob, marketMaker } = c;
  ...
  it("cash settlement excludes the market maker's pending payout", async () => {
    const { hub, weth, usdc, alice, marketMaker } = c;
    const { vaultId, vaultAddress, bid } = await goLive(
      c,
      { withFeed: true },
    ...
    await expect(hub.connect(marketMaker).claimPayout(vaultId)).to.changeTokenBalances(
      ethers,
      weth,
      [marketMaker],
      [CALL_PAYOUT_ALL],
  ...
  it("transferred shares can claim", async () => {
    const { hub, shares, weth, usdc, alice, carol } = c;
    const { vaultId } = await expiredCall(c);
  ...
  it("partial claims stay proportional", async () => {
    const { hub, shares, weth, usdc, alice } = c;
    const { vaultId } = await expiredCall(c);
  ...
  it("rejects zero, too many shares and the wrong phase", async () => {
    const { hub, alice, carol } = c;
    const { vaultId } = await expiredCall(c);
    ...
    await expect(hub.connect(alice).claim(vaultId, 7n * WETH_UNIT)).to.be.revertedWithCustomError(
      hub,
      "InsufficientShares",
    );
    const live = await goLive(c);
    await expect(hub.connect(alice).claim(live.vaultId, 1n))
