	...
	describe("approve", () => {
		context("with a recorded proposal", () => {
			...
			it("rejects approving from anyone but the hub", async () => {
				await expect(c.unwind.connect(c.stranger).approve(VAULT_ID, 1n, c.holder.address, 1n)).to.be.revertedWithCustomError(c.unwind, "NotHub")
			})

			it("rejects a holder with a zero balance", async () => {
				await expect(c.unwind.connect(c.hub).approve(VAULT_ID, 1n, c.holder.address, 0n)).to.be.revertedWithCustomError(c.unwind, "AgreementInvalid")
			})
		})
	})
	...
	describe("revoke", () => {
		context("before any proposal", () => {
			...
			it("reports no approval change for a holder who never approved", async () => {
				await expect(c.unwind.connect(c.hub).revoke(VAULT_ID, c.holder.address)).not.to.emit(c.unwind, "ApprovalUpdated")
			})
		})
		...
		context("with a recorded proposal", () => {
			...
			it("rejects revoking from anyone but the hub", async () => {
				await expect(c.unwind.connect(c.stranger).revoke(VAULT_ID, c.holder.address)).to.be.revertedWithCustomError(c.unwind, "NotHub")
			})
		})
	})
	...
	describe("fund", () => {
		context("with a recorded proposal", () => {
			...
			it("rejects funding from anyone but the hub", async () => {
				await expect(c.unwind.connect(c.stranger).fund(VAULT_ID, 1n, c.holder.address, 1n, 0n, 0n)).to.be.revertedWithCustomError(c.unwind, "NotHub")
			})
		})
	})
	...
	describe("withdraw", () => {
		context("with a recorded proposal", () => {
			...
			it("rejects withdrawing from anyone but the hub", async () => {
				await expect(c.unwind.connect(c.stranger).withdraw(VAULT_ID, 1n, c.holder.address)).to.be.revertedWithCustomError(c.unwind, "NotHub")
			})
		})
	})
	...
	describe("requiredContribution", () => {
		context("before any proposal", () => {
			...
			it("reverts with AgreementInvalid", async () => {
				await expect(c.unwind.requiredContribution(VAULT_ID, weth(1))).to.be.revertedWithCustomError(c.unwind, "AgreementInvalid")
			})
		})
