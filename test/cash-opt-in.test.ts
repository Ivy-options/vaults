...
// Each variant gets its own chain: snapshots on one chain are stacked.
const disabledChain = await network.create();
const enabledChain = await network.create();
const loadCashDisabled = fixture(disabledChain, () => deployIvy(disabledChain, { enableCashSettlement: false }));
const loadCashEnabled = fixture(enabledChain, () => deployIvy(enabledChain));

describe("Cash settlement opt-in", () => {
  let c: IvyContext;

  describe("deployed with cash disabled", () => {
    beforeEach(async () => {
      c = await loadCashDisabled();
    });

    it("deploys with cash disabled and no settlement publishers", async () => {
      ...
      await expect(c.hub.publishExercisePrice(1n, 4000n * U, now, now + 100n)).revertedWithCustomError(
        c.hub,
    ...
    it("keeps publisher-role administration guarded and disallows a zero-address publisher", async () => {
      const role = await c.hub.SETTLEMENT_PRICE_PUBLISHER_ROLE();
      await expect(c.hub.connect(c.bob).grantRole(role, c.bob.address)).revertedWithCustomError(
        c.hub,
        "AccessControlUnauthorizedAccount",
      );
      ...
      await c.hub.grantRole(role, c.bob.address);
      expect(await c.hub.cashSettlementEnabled()).equal(false);
      await expect(c.hub.connect(c.carol).renounceRole(role, c.bob.address)).revertedWithCustomError(
        c.hub,
    ...
    it("explicitly enables cash with an EOA or authenticated helper available and preserves physical-only terms", async () => {
      const role = await c.hub.SETTLEMENT_PRICE_PUBLISHER_ROLE();
      const physical = await createVaultAs(c, c.alice, callTerms(c), callPairs(c), []);
      await c.hub.grantRole(role, c.bob.address);
      await c.hub.setCashSettlementEnabled(true);
      for (const policy of [SettlementPolicy.Cash, SettlementPolicy.Either]) {
        await c.hub.createVault(
          callTerms(c, { allowedSettlement: policy, maxSettlementPriceAge: 3600 }),
          callPairs(c),
          [],
        );
        await expect(
          c.hub.createVault(callTerms(c, { allowedSettlement: policy }), callPairs(c), []),
      ...
      const now = BigInt(await c.networkHelpers.time.latest());
      await expect(
        helper.connect(c.carol).publishExercisePrice(v.vaultId, 4000n * U, now, now + 100n),
      ...
      await expect(
        c.hub.createVault(
          callTerms(c, { allowedSettlement: SettlementPolicy.Cash, maxSettlementPriceAge: 3600 }),
          callPairs(c),
          [],
        ),
      ).revertedWithCustomError(c.hub, "CashSettlementDisabled");
    ...
    for (const isCall of [true, false]) {
      it(`completes the physical ${isCall ? "call" : "put"} lifecycle without any settlement publisher or indicative feed`, async () => {
        ...
        await expect(c.hub.connect(c.alice).claim(v.vaultId, isCall ? 10n * W : 30_000n * U)).changeTokenBalance(
          c.ethers,
          isCall ? c.usdc : c.weth,
          c.alice,
          isCall ? 30_000n * U : 10n * W,
        );
  ...
  describe("deployed with cash enabled", () => {
    beforeEach(async () => {
      c = await loadCashEnabled();
    });

    it("rejects pending cash bids while the feature flag is disabled without consuming the bid, and accepts them after re-enabling", async () => {
      const role = await c.hub.SETTLEMENT_PRICE_PUBLISHER_ROLE();
      const v = await openVault(c, {
        terms: { allowedSettlement: SettlementPolicy.Cash, maxSettlementPriceAge: 3600 },
      });
      ...
      await expect(c.hub.connect(c.bidMaster).activate(v.vaultId, bid, signature)).revertedWithCustomError(
        c.hub,
    ...
    it("still activates physical bids in existing Either auctions while the feature flag is disabled", async () => {
      const v = await openVault(c, {
        terms: { allowedSettlement: SettlementPolicy.Either, maxSettlementPriceAge: 3600 },
      });
      await c.hub.setCashSettlementEnabled(false);
    ...
    it("preserves live cash publication, exercise, finalization and claims while admissions are disabled", async () => {
      const role = await c.hub.SETTLEMENT_PRICE_PUBLISHER_ROLE();
      ...
      await c.hub.revokeRole(role, c.admin.address);
      expect(await c.hub.cashSettlementEnabled()).equal(false);
      await expect(c.hub.connect(c.marketMaker).exercise(v.vaultId, W)).changeTokenBalance(
        c.ethers,
        c.weth,
        c.marketMaker,
        W / 4n,
    ...
    it("keeps cash reports pending while disabled and recovers through regrant before the publication deadline", async () => {
      const role = await c.hub.SETTLEMENT_PRICE_PUBLISHER_ROLE();
      await c.hub.setPlatformFeeBps(200);
      const v = await goLive(
        c,
        { isCall: false, terms: { allowedSettlement: SettlementPolicy.Cash, maxSettlementPriceAge: 3600 } },
        { settlement: SettlementType.Cash },
      );
      await c.hub.setCashSettlementEnabled(false);
      await c.hub.revokeRole(role, c.admin.address);
