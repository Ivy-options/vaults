...
library BidHash {
    bytes32 internal constant BID_TYPEHASH = keccak256(
        "Bid(uint256 vaultId,address marketMaker,address quoteToken,uint256 strike,uint256 premium,uint8 style,uint8 settlement,uint64 expiry,uint64 validUntil,uint256 nonce,uint256 auctionId,uint256 collateralAmount,bytes32 pairHash,address executor,address recipient)"
    );
